 ...
 * MA  02110-1301, USA.
 */
package org.eclipse.ceylon.itest;
...
public class CeylonCompileAntTests extends AntBasedTests {

    public CeylonCompileAntTests() throws Exception {
        super("test/src/org/eclipse/ceylon/itest/ceylon-compile-ant.xml");
    }
    ...
    public void testCompileFileFromFooTwice() throws Exception {
        ...
        Assert.assertEquals(0, result.getStatusCode());
        assertContainsMatch(result.getStdout(), Pattern.compile("^\\[ceylon-compile\\] No need to compile .*?/test/src/org/eclipse/ceylon/itest/com/example/foo/a/foo.ceylon, it's up to date$", Pattern.MULTILINE));
        assertContains(result.getStdout(), "[ceylon-compile] Everything's up to date");
        Assert.assertEquals(lastModified, car.lastModified());
    }
