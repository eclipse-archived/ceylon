...
import org.eclipse.ceylon.compiler.typechecker.tree.Tree.AnyClass;
...
import org.eclipse.ceylon.langtools.tools.javac.main.Main.Result;
    ...
    private Runnable getModuleInitialiser(final String moduleName, final URL[] carUrls, final String[] dependencies, final Set<String> removeAtRuntime, final ClassLoader cl) throws URISyntaxException{
        ...
        return new Runnable(){
            ...
            public void run() {
                ...
                Metamodel.loadModule("org.eclipse.ceylon.typechecker", TypeChecker.LANGUAGE_MODULE_VERSION, CompilerTests.makeArtifactResult(new File("../dist/dist/repo/org/eclipse/ceylon/typechecker/"+TypeChecker.LANGUAGE_MODULE_VERSION+"/org.eclipse.ceylon.typechecker-"+TypeChecker.LANGUAGE_MODULE_VERSION+".jar")), cl);
                Metamodel.loadModule("org.eclipse.ceylon.model", TypeChecker.LANGUAGE_MODULE_VERSION, CompilerTests.makeArtifactResult(new File("../dist/dist/repo/org/eclipse/ceylon/model/"+TypeChecker.LANGUAGE_MODULE_VERSION+"/org.eclipse.ceylon.model-"+TypeChecker.LANGUAGE_MODULE_VERSION+".jar")), cl);
                    ...
                    public void visit(Declaration that) {
                        ...
                        for (CompilerAnnotation ca : that.getCompilerAnnotations()) {
                            ...
                            if ("test".equals(identifier.getToken().getText())) {
                                final org.eclipse.ceylon.model.typechecker.model.Declaration decl = that.getDeclarationModel();
                                if (moduleName.isEmpty() || ModelUtil.getModule(decl).getNameAsString().equals(moduleName)) {
                                    boolean added = false;
                                    if (testClassName != null || decl.isToplevel()) {
                                        if (decl instanceof org.eclipse.ceylon.model.typechecker.model.Function) {
                                            org.eclipse.ceylon.model.typechecker.model.Function method = (org.eclipse.ceylon.model.typechecker.model.Function)decl;
