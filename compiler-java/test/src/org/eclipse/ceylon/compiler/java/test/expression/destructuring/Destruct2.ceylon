...
void tupleVar([Integer, Float, String] tuple) {
    let ([i1, f1, s1] = tuple);
    let ([i2, Float f2, s2] = tuple);
    let ([Integer i3, Float f3, String s3] = tuple);
}

@noanno
void tupleLiteral() {
    let ([i1, f1, s1] = [0, 1.0, "foo"]);
    let ([i2, Float f2, s2] = [0, 1.0, "foo"]);
    let ([Integer i3, Float f3, String s3] = [0, 1.0, "foo"]);
}
...
void tupleGeneric() {
    ...
    let ([f1] = [FooSub<Integer>()]);
    let ([FooSub<Integer> f2] = [FooSub<Integer>()]);
    let ([Foo<Integer> f3] = [FooSub<Integer>()]);
}

@noanno
void entryVar(Integer->String entry) {
    let (i1->s1 = entry);
    let (Integer i2->s2 = entry);
    let (Integer i3->String s3 = entry);
}

@noanno
void entryLiteral() {
    let (i1->s1 = 0->"foo");
    let (Integer i2->s2 = 0->"foo");
    let (Integer i3->String s3 = 0->"foo");
}
...
void entryGeneric() {
    class Foo<T>() {}
    class FooSub<T>() extends Foo<T>() {}
    let (i1->f1 = 0->FooSub<Integer>());
    let (Integer i2->FooSub<Integer> f2 = 0->FooSub<Integer>());
    let (Integer i3->Foo<Integer> f3 = 0->FooSub<Integer>());
}
