...
shared void run(String expectedVersion) {
    ...
    //assert("1.2.3 (Contents May Differ)" == Versions.\iCEYLON_VERSION);
    ...
    assert("module ceylon.language/"+expectedVersion == `module ceylon.language`.string);
    //XXX see ceylon-compiler#2428 print("\`module ceylon.runtime\` is `` `module ceylon.runtime` ``");
    assert("module org.eclipse.ceylon.common/"+expectedVersion == `module org.eclipse.ceylon.common`.string);
    ...
    assert("module org.eclipse.ceylon.compiler.java/"+expectedVersion == `module org.eclipse.ceylon.compiler.java`.string);
    assert("module org.eclipse.ceylon.compiler.js/"+expectedVersion == `module org.eclipse.ceylon.compiler.js`.string);
    ...
    for (imp in `module`.dependencies) {
        ...
        assert(expectedVersion == imp.version);
    }
