...
public class ExpressionTransformer extends AbstractTransformer {
	...
	/**
	 * This implies inclusion of the JT_SATISFIES flags when constructing the type
	 * for a variance typecast.
	 */
	public static final int EXPR_FOR_COMPANION = 1;
	/**
	 * The expected type has type parameters (so an extra typecast to the raw type
	 * will be required)
	 */
	public static final int EXPR_EXPECTED_TYPE_NOT_RAW = 1 << 1;
	/**
	 * The expected type has type parameters with {@code satisfies} constraints
	 * (which may be erased, and thus a type cast may be required irrespective of
	 * the presence of type arguments)
	 */
	public static final int EXPR_EXPECTED_TYPE_HAS_CONSTRAINED_TYPE_PARAMETERS = 1 << 2;
	/**
	 * Seems to be used when the expected and expression types have no supertype in
	 * common.
	 */
	public static final int EXPR_DOWN_CAST = 1 << 3;
	/**
	 * Use this when the expression being passed contains code to check for nulls
	 * coming from Java
	 */
	public static final int EXPR_HAS_NULL_CHECK_FENCE = 1 << 4;
	/**
	 * This implies inclusion of the JT_COMPANION flags when constructing the type
	 * casts.
	 */
	...
	/**
	 * The expected type has type parameters with {@code satisfies} constraints
	 * which have a covariant type parameter that is used by other type parameter
	 * bounds, so we will always generate types where it is fixed rather than using
	 * wildcards and in some cases we need to cast to raw
	 */
	 ...
	 * conversion is required we use a Util invocation so that a runtime check is
	 * performed.
	 * 
	 * In some circumstances (e.g. annotations) we need to use a real typecast.
	 */
	public static final int EXPR_UNSAFE_PRIMITIVE_TYPECAST_OK = 1 << 7;

	/**
	 * Use this when the expression is to be passed to a Java field/setter/parameter
	 * which does not require null-safety
	 */
	...
	/**
	 * Use this when the expression is guaranteed to not be a base member. This
	 * allows us to differentiate `null` (BaseMemberExpression) from other things of
	 * type "null" which get stored in temp vars erased to `Object`, for erasure and
	 * casts.
	 */
	...
	static {
		// only there to make sure this class is initialised before the enums defined in
		// it, otherwise we
	...
	/**
	 * The transformation for spread method references involves nested invocations
	 * of
	 * {@link #transformSpreadOperator(org.eclipse.ceylon.compiler.typechecker.tree.Tree.QualifiedMemberExpression, TermTransformer)}
	 * and what we generate depends on the invocation. This field will be odd on the
	 * "outer" invocation (which generates the outer part of the tree) and even on
	 * the "inner" invocation (which generates the inner part of the tree)
	 */
	...
	/**
	 * This is set when we're coercing a FunctionArgument to an FI that doesn't care
	 * about null returns
	 */
	...
	public JCStatement transform(Tree.ExpressionStatement tree) {
		// ExpressionStatements do not return any value, therefore we don't care about
	...
	public JCStatement transform(Tree.SpecifierStatement op) {
		// SpecifierStatement do not return any value, therefore we don't care about the
		...
		if (error != null) {
			result = this.makeThrowUnresolvedCompilationError(error);
				...
				.getFirstExpressionErrorAndMarkBrokenness(op.getSpecifierExpression().getExpression())) != null) {
			result = this.makeThrowUnresolvedCompilationError(error);
		} else {
			result = at(op).Exec(
					transformAssignment(op, op.getBaseMemberExpression(), op.getSpecifierExpression().getExpression()));
		}
		inStatement = false;
		return result;
	}

	public JCExpression transform(Tree.SpecifierOrInitializerExpression expr, BoxingStrategy boxing,
			Type expectedType) {
		return transformExpression(expr.getExpression(), boxing, expectedType);
	}

	public JCExpression transform(Tree.SpecifierOrInitializerExpression expr, TypedDeclaration decl) {
		// TODO: is there missing logic for small here?
		// see ClassTransformer.transform(AttributeDeclaration decl,
		// ClassDefinitionBuilder classBuilder)
		return transformExpression(expr.getExpression(), CodegenUtil.getBoxingStrategy(decl), decl.getType(),
	...
	JCExpression transformExpression(final Tree.Term expr, BoxingStrategy boxingStrategy, Type expectedType,
		...
		if ((flags & EXPR_TARGET_ACCEPTS_NULL) == 0 && expectedType != null && hasUncheckedNulls(expr)
				&& expectedType.isSubtypeOf(typeFact().getObjectType()) && !knownNullSafe(term)) {
	...
	private static java.util.List<String> knownNullSafe = Arrays.asList("java.util::Objects.toString",
			"java.util::Arrays.asList", "java.util::Arrays.copyOf", "java.util::Arrays.copyOfRange",
			"java.util::Arrays.deepToString", "java.util::Collections.unmodifiableList",
			...
			"java.util::Collections.synchronizedSet", "java.util::Collections.synchronizedSortedMap",
			"java.util::Collections.synchronizedSortedSet", "java.util::Collections.emptyIterator",
			"java.util::Collections.emptyListIterator", "java.util::Collections.emptyList",
			"java.util::Collections.emptyMap", "java.util::Collections.emptySet",
			"java.util::Collections.emptySortedMap", "java.util::Collections.emptySortedSet",
			"java.util::Collections.singleton", "java.util::Collections.singletonList",
			"java.util::Collections.singletonMap", "java.lang::String.substring", "java.lang::String.toLowerCase",
			"java.lang::String.toUpperCase", "java.lang::String.intern", "java.lang::String.replace",
			"java.lang::String.replaceAll", "java.lang::String.replaceFirst", "java.lang::String.toCharArray",
			"java.lang::String.getBytes", "java.lang::String.getChars", "java.lang::String.trim",
			"java.lang::String.format", "java.lang::String.join", "java.lang::String.split",
			"java.lang::String.copyValueOf", "java.lang::String.valueOf", "java.lang::Double.valueOf",
			"java.lang::Float.valueOf", "java.lang::Long.valueOf", "java.lang::Integer.valueOf",
			"java.lang::Short.valueOf", "java.lang::Byte.valueOf", "java.lang::Boolean.valueOf",
			"java.lang::Character.valueOf", "java.lang::Double.toString", "java.lang::Float.toString",
			"java.lang::Long.toString", "java.lang::Integer.toString", "java.lang::Short.toString",
			"java.lang::Byte.toString", "java.lang::Boolean.toString", "java.lang::Character.toString");
	...
	JCExpression transform(Tree.FunctionArgument functionArg) {
		...
		int flags = 0;
		// even if we are a Callable<X>, if the FI returns void don't check for non-null
		// return types
		...
		try {
			...
			expressionGen().coercedFunctionalInterfaceNeedsNoNullChecks = (flags & EXPR_TARGET_ACCEPTS_NULL) != 0
					? model
					: null;
			if (functionArg.getBlock() != null) {
				...
				if (!functionArg.getBlock().getDefinitelyReturns()) {
					if (isAnything(model.getType())) {
						...
						body = body.append(make().Return(makeErroneous(functionArg.getBlock(),
								"compiler bug: non-void method does not definitely return")));
					}
				}
			} else {
		...
		CallableBuilder callableBuilder = CallableBuilder.methodArgument(gen(), functionArg, model, callableType,
	...
	private JCExpression applyErasureAndBoxing(JCExpression result, Tree.Term expr, BoxingStrategy boxingStrategy,
		...
		return applyErasureAndBoxing(result, exprType, exprErased, exprBoxed, exprUntrustedType, exprSmall,
				boxingStrategy, expectedType, flags);
	}

	JCExpression applyErasureAndBoxing(JCExpression result, Type exprType, boolean exprBoxed,
	...
	JCExpression applyErasureAndBoxing(JCExpression result, Type exprType, boolean exprErased, boolean exprBoxed,
			BoxingStrategy boxingStrategy, Type expectedType, int flags) {
		return applyErasureAndBoxing(result, exprType, exprErased, exprBoxed, false, boxingStrategy, expectedType,
				flags);
	}

	JCExpression applyErasureAndBoxing(JCExpression result, Type exprType, boolean exprErased, boolean exprBoxed,
			boolean exprUntrustedType, BoxingStrategy boxingStrategy, Type expectedType, int flags) {
		return applyErasureAndBoxing(result, exprType, exprErased, exprBoxed, exprUntrustedType, false, boxingStrategy,
				expectedType, flags);
	}

	JCExpression applyErasureAndBoxing(JCExpression result, Type exprType, boolean exprErased, boolean exprBoxed,
			boolean exprUntrustedType, boolean exprSmall, BoxingStrategy boxingStrategy, Type expectedType, int flags) {
		...
		if (expectedType != null
			...
			if (exprBoxed) {
				...
				boolean expectedTypeHasConstrainedTypeParameters = (flags
						& EXPR_EXPECTED_TYPE_HAS_CONSTRAINED_TYPE_PARAMETERS) != 0;
				boolean expectedTypeHasDependentCovariantTypeParameters = (flags
						& EXPR_EXPECTED_TYPE_HAS_DEPENDENT_COVARIANT_TYPE_PARAMETERS) != 0;
				...
				if (isNull(exprType)) {
					...
					if (!isNullValue(exprType)
							...
							// include a cast even for null for interop and disambiguating bw overloads and
							// null values
							// of different types using the "of" operator
							|| downCast
							// also add casts if we really want them
							|| forceCast) {
						// in some cases we may have an instance of Null, which is of type
						// java.lang.Object, being
						// returned in a context where we expect a String? (aka ceylon.language.String)
						// so even though
						// the instance at hand will really be null, we need a up-cast to it
						JCExpression targetType = makeJavaType(expectedType,
								AbstractTransformer.JT_RAW | companionFlags);
				...
				exprErased
						// we want to cast
						|| forceCast
						// expression type cannot be trusted to be true, most probably because we had to
						// satisfy Java type parameter
						// bounds that are different from what we think the expression type should be
						|| exprUntrustedType
						// if we have a covariant type parameter which is dependent and whose type arg
						// contains erased type parameters
						// we need a raw cast because it will be fixed rather than using a wildcard and
						...
						|| needsCast(exprType, expectedType, expectedTypeIsNotRaw,
								expectedTypeHasConstrainedTypeParameters, downCast)
					...
					if (forceCast && !expectedType.isTypeParameter() && !exprType.isSubtypeOf(expectedType)
					...
					// if the expr is raw:
					// don't even try making an actual cast if there are bounded type parameters in
					// play, because going raw is much safer
					// also don't try making the cast if the expected type is raw because anything
					// goes
					boolean needsTypedCast = !exprIsRaw || (!expectedTypeHasConstrainedTypeParameters
							&& !expectedTypeHasDependentCovariantTypeParameters && !expectedTypeIsRaw);
					if (needsTypedCast
							// make sure that downcasts get at least one cast
							|| downCast
							// same for forced erasure
							|| exprUntrustedType) {
						// forced erasure may require a previous cast to Object if we were not able to
						// insert a raw cast
						// because for instance Sequential<String> cannot be cast forcibly to Empty
						// because Java is so smart
						// it figures out that there's no intersection between the two types, but we
						// know better
		...
		// If expr type if Self<T> and expected type is T we need to cast before any
		// unboxing
		boolean mightNeedCastToSelfType = expectedType != null && exprBoxed
		...
		if (mightNeedCastToSelfType) {
			...
			if (selfType != null && expectedType.isExactly(selfType)) {
				result = applySelfTypeCasts(result, exprType, exprBoxed, BoxingStrategy.BOXED, expectedType,
						getSelfTypeParam(exprType));
				exprType = expectedType;
			}
		}
		// we must do the boxing after the cast to the proper type
		JCExpression ret = boxUnboxIfNecessary(result, exprBoxed ? BoxingStrategy.BOXED : BoxingStrategy.UNBOXED,
				exprType, boxingStrategy, expectedType);
		...
		// ret = applySelfTypeCasts(ret, exprType, exprBoxed, boxingStrategy,
	...
	private JCExpression applyJavaCoercions(JCExpression ret, Type exprType, Type expectedType) {
		...
		if (isCeylonString(exprType) && isJavaCharSequence(expectedType)) {
			...
			if (isOptional(nonSimpleExprType)) {
				...
				JCExpression convert = make().Apply(null, makeQualIdent(varName.makeIdent(), "toString"),
		...
		if (isCeylonArray(exprType) && isJavaArray(expectedType)) {
			...
			if (isOptional(nonSimpleExprType)) {
				Naming.SyntheticName varName = naming.temp();
				JCExpression test = make().Binary(JCTree.Tag.NE, varName.makeIdent(), makeNull());
				JCExpression convert = make().Apply(null, makeQualIdent(varName.makeIdent(), "toArray"),
						List.<JCTree.JCExpression>nil());
				JCExpression cond = make().Conditional(test, convert, makeNull());
				JCExpression typeExpr = makeJavaType(typeFact().getObjectType());
		...
		if (isCeylonClassOrInterfaceModel(exprType) && isJavaClass(expectedType)
			...
			// into
			// Util.classErasure(.org.eclipse.ceylon.compiler.java.test.interop.LambdasJava.class)
			...
			if (arg instanceof JCTree.JCMethodInvocation) {
				...
				if (methodSelect instanceof JCTree.JCFieldAccess) {
					...
					if (methodField.getIdentifier().toString().equals("typeLiteral")
							...
							&& ((JCTree.JCFieldAccess) methodField.getExpression()).toString()
									.equals(".ceylon.language.meta.typeLiteral_")) {
						JCExpression classLiteral = extractClassLiteralFromTypeDescriptor(
								((JCTree.JCMethodInvocation) arg).getArguments().get(0));
			...
			if (willEraseToObject(exprType))
				ret = make().TypeCast(makeJavaType(typeFact().getClassOrInterfaceModelType(typeFact().getObjectType())),
						ret);
			// FIXME: pass type arg explicitly?
		...
		if (arg instanceof JCTree.JCMethodInvocation) {
			...
			if (methodSelect instanceof JCTree.JCFieldAccess) {
				...
				if (methodField.getIdentifier().toString().equals("klass")
						&& methodField.getExpression() instanceof JCTree.JCFieldAccess
						&& ((JCTree.JCFieldAccess) methodField.getExpression()).toString()
								.equals(".org.eclipse.ceylon.compiler.java.runtime.model.TypeDescriptor")) {
				...
				if (methodField.getIdentifier().toString().equals("member")
						...
						&& ((JCTree.JCFieldAccess) methodField.getExpression()).toString()
								.equals(".org.eclipse.ceylon.compiler.java.runtime.model.TypeDescriptor")) {
					return extractClassLiteralFromTypeDescriptor(
							((JCTree.JCMethodInvocation) arg).getArguments().get(1));
				}
			}
		}
		return null;
	...
	boolean needsCast(Type exprType, Type expectedType, boolean expectedTypeNotRaw,
			boolean expectedTypeHasConstrainedTypeParameters, boolean downCast) {
		// error handling
		if (exprType == null)
			return false;
		...
		if (exprType.isExactly(expectedType)) {
			// unless the expected type is parameterised with bounds because in that case we
			// can't
		...
		if (eraseExpectedType) {
			// unless the expected type is parameterised with bounds that erasure to Object
		...
		if (commonType == null || !(commonType.getDeclaration() instanceof ClassOrInterface)) {
			// we did not find any common type, but we may be downcasting, in which case we
			// need a cast
			return downCast;
		}

		// some times we can lose info due to an erased type parameter somewhere in the
		// inheritance graph
		if (lostTypeParameterInInheritance(exprType, commonType))
			return true;

		if (!expectedTypeNotRaw) {
			// the truth is that we don't really know if the expected type is raw or not,
			// that flag only gets set
			// if we know for sure that the expected type is NOT raw. if it's false we've no
			// idea but we can check:
			if (isTurnedToRaw(expectedType)) {
				return false;
			}
			// if the expected type is exactly the common type, they must have the same
			// erasure
			// note that we don't do that test if we know the expected type is not raw,
			// because
			// the common type could be erased
			if (commonType.isExactly(expectedType))
				return false;
		}
		// special case for Callable because only the first type param exists in Java,
		// the rest is completely suppressed
		...
		if (commonType.getQualifyingType() != null && expectedType.getQualifyingType() != null) {
			return needsCast(commonType.getQualifyingType(), expectedType.getQualifyingType(), expectedTypeNotRaw,
					expectedTypeHasConstrainedTypeParameters, downCast);
		}
		return false;
	}
	...
	private boolean lostTypeParameterInInheritance(Type exprType, Type commonType) {
		...
		// do not search interfaces if the common declaration is a class, because
		// interfaces cannot be subtypes of a class
	...
	private boolean lostTypeParameterInInheritance(ClassOrInterface exprDecl, ClassOrInterface commonDecl,
			boolean searchInterfaces, boolean lostTypeParameter) {
		...
		if (searchInterfaces) {
			...
			for (Type pt : exprDecl.getSatisfiedTypes()) {
				// FIXME: this is very heavy-handed because we consider that once we've lost a
				// type parameter we've lost them all
				// but we could optimise this by checking:
				// 1/ which type parameter we've really lost
				// 2/ if the type parameters we're passing to our super type actually depend in
				// any way from type parameters we've lost
		...
		if (extendedType != null) {
			...
			if (extendedTypeDeclaration != typeFact().getObjectDeclaration())
				return lostTypeParameterInInheritance(extendedTypeDeclaration, commonDecl, searchInterfaces,
						lostTypeParameter2);
		}
		// didn't find it
		return false;
	...
	private JCExpression applyVarianceCasts(JCExpression result, Type exprType, boolean exprBoxed,
			BoxingStrategy boxingStrategy, Type expectedType, int flags) {
	...
	private JCExpression applyVarianceCasts(JCExpression result, Type expectedType,
			VarianceCastResult varianceCastResult, int flags) {
		// Types with variance types need a type cast, let's start with a raw cast to
		// get rid
		// of Java's type system constraint (javac doesn't grok multiple implementations
		// of the same
		...
		if (varianceCastResult.isBetterCastAvailable()) {
			...
			targetType = makeJavaType(varianceCastResult.castType,
					AbstractTransformer.JT_TYPE_ARGUMENT | wantsCompanionMask | forCompanionMask);
			result = make().TypeCast(targetType, result);
		}
		return result;
	}

	private JCExpression applySelfTypeCasts(JCExpression result, Type exprType, boolean exprBoxed,
			BoxingStrategy boxingStrategy, Type expectedType, final TypeParameter selfTypeParam) {
		...
		if (selfTypeParam.getType().isExactly(exprType) // self-type within its own scope
			...
			// the fact that the original expr was or not boxed doesn't mean the current
			// result is boxed or not
	...
	private JCExpression applyJavaTypeConversions(JCExpression ret, Type exprType, Type expectedType,
		...
		if (definiteExprType == null)
			return ret;
		// ignore the underlying type of the expr type if it was boxed, since we must
		// have unboxed it to
		...
		if (isCeylonByte(definiteExpectedType) && isCeylonInteger(exprType)) {
			if ((flags & EXPR_UNSAFE_PRIMITIVE_TYPECAST_OK) == 0) {
				if (ret instanceof JCTree.JCUnary) {
					...
					if (unary.getTag() == JCTree.Tag.NEG && unary.arg instanceof JCTree.JCLiteral) {
						Object value = ((JCTree.JCLiteral) unary.arg).value;
						if (value instanceof Integer) {
							int val = (Integer) value;
							// if it fits let's just leave it
							if (val >= 0 && val <= -Byte.MIN_VALUE) {
								// in the case of -128 to 127 we don't need to cast to byte by using an int
								// literal, but only for
			...
			if (convertTo != null) {
				...
				if (isCeylonInteger(exprType)
						&& (convertFrom.equals("int") || convertFrom.equals("short") || convertFrom.equals("byte"))) {
	...
	private final class InvocationTermTransformer implements TermTransformer {
		...
		public JCExpression transform(JCExpression primaryExpr, String selector) {
			...
			if (invocation instanceof NamedArgumentInvocation) {
				return transformNamedArgumentInvocationOrInstantiation((NamedArgumentInvocation) invocation,
						callBuilder, transformedPrimary);
			} else {
	...
	static int literalValue(Tree.CharLiteral ch) {
		// codePoint is at index 1 because the text is `X` (including quotation marks,
		// so we skip them)
	...
	static double literalValue(Tree.FloatLiteral literal) throws ErroneousException {
		...
		if (value == Double.POSITIVE_INFINITY) {
			throw new ErroneousException(literal, "literal so large it is indistinguishable from infinity: '"
					+ literal.getText() + "' (use infinity)");
		} else if (value == 0.0 && !literal.getText().equals("0.0")) {
			literal.addUsageWarning(Warning.zeroFloatLiteral,
					"literal so small it is indistinguishable from zero: '" + literal.getText() + "' (use 0.0)");
		}
		return value;
	}
	...
	static private Number literalValue(Tree.NaturalLiteral literal, String text) throws ErroneousException {
		...
		if (text.startsWith("#")) {
			...
			try {
				...
				throw new ErroneousException(literal, "literal outside representable range: '" + text
	...
	/**
	 * Returns the literal value of the given literal in the given radix, or throws
	 * an ErroneousException with the given error message if the literal cannot be
	 * represented.
	 * 
	 * @return An Integer (if the literal can be represented as an Integer) or Long
	 *         (if the literal cannot be represented as an Integer, but can be
	 *         represented as a Long)
	 * @throws ErroneousException
	 *             If the literal cannot be represented as a long
	 */
	...
	public JCTree transform(Tree.MemberLiteral expr) {
		...
		if (declaration.isToplevel()) {
			...
			if (objectMember) {
				// We don't care about the type args for the cast, nor for the reified container
				// expr, because
				// we take the real reified container type from the container instance, and that
				// one has the type
				// arguments
			...
			if (declaration.isConstructor()) {
				...
				metatypeName = constructedClass.isToplevel() || !(container instanceof TypeDeclaration) ? "Class"
						: "MemberClass";
			} else {
				metatypeName = "ClassOrInterface";
			}
			TypeDeclaration classOrInterfaceDeclaration = (TypeDeclaration) typeFact()
					.getLanguageModuleModelDeclaration(metatypeName);
			...
			// Note that we don't use Basic for the container for object members, because
			// that's not how we represent
			// anonymous types.
			...
			if (declaration.isConstructor()) {
				...
				/*
				 * JCExpression reifiedArgumentsExpr; if
				 * (Decl.isEnumeratedConstructor(Decl.getConstructor(declaration))) {
				 * reifiedArgumentsExpr =
				 * makeReifiedTypeArgument(typeFact().getCallableTuple(callableType.
				 * getQualifyingType())); } else { reifiedArgumentsExpr =
				 * makeReifiedTypeArgument(typeFact().getCallableTuple(callableType)); }
				 */
				JCExpression reifiedArguments = getConstructor(declaration).isValueConstructor()
				...
				Type callableType = producedReference.getFullType();
				JCExpression reifiedReturnTypeExpr = makeReifiedTypeArgument(
						typeFact().getCallableReturnType(callableType));
				...
				memberCall = make().Apply(null, makeSelect(typeCall, getterName), List.of(reifiedContainerExpr,
						reifiedGetExpr, reifiedSetExpr, ceylonLiteral(declaration.getName())));
			} else {
			...
			// memberCall = at(expr).Apply(null, makeSelect(memberCall, "bind"),
			// List.of(object));
			// }
	...
	JCExpression makeMemberValueOrFunctionDeclarationLiteral(Tree.MemberLiteral expr, Declaration declaration) {
		...
		return declaration.isConstructor() ? makeConstructorDeclarationLiteral(exprType, declaration)
	...
	JCExpression makeConstructorDeclarationLiteral(Type type, Declaration declaration) {
		...
		JCExpression metamodelCall = make().TypeCast(metamodelCast, makeTypeDeclarationLiteral(clas));

		JCExpression typeClass = makeJavaType(type, JT_NO_PRIMITIVES);
		JCExpression call = make().Apply(null, makeSelect(metamodelCall, "getConstructorDeclaration"),
	...
	JCExpression makeMemberValueOrFunctionDeclarationLiteral(Type type, Declaration declaration,
		...
		JCExpression metamodelCast = makeJavaType(classDecType, JT_NO_PRIMITIVES);
		JCExpression metamodelCall = make().TypeCast(metamodelCast, makeTypeDeclarationLiteral(classOrInterface));
		...
		return make().Apply(List.of(memberType), makeSelect(metamodelCall, memberAccessor),
				List.of(reifiedMemberType, ceylonLiteral(name)));
	}

	JCExpression makeLocalValueOrFunctionDeclarationLiteral(Declaration declaration, boolean mightBeInherited) {
		String name = declaration.getName();
		...
		return make().TypeCast(makeJavaType(typeFact().getGenericDeclarationType()),
				make().Apply(null, makeSelect(metamodelCall, memberAccessor),
						List.of(makeReifiedTypeArgument(typeFact().getAnythingType()), ceylonLiteral(name))));
	}
	...
	private JCTree makeTopLevelValueOrFunctionLiteral(Tree.MemberLiteral expr) {
		...
		if (!expr.getWantsDeclaration()) {
			...
			// expr is of type Function<Type,Arguments> or Value<Get,Set> so we can get its
			// type like that
			...
			if (Decl.isMethod(declaration)) {
				// expr is of type Function<Type,Arguments> so we can get its arguments type
				// like that
				...
				Type ptype = !((Value) declaration).isVariable() ? typeFact().getNothingType()
	...
	private JCExpression makeModuleLiteralCall(Module module) {
		...
		if (module.isDefaultModule()) {
			...
			call = make().Apply(null, makeSelect(modulesGet, "find"),
					List.<JCExpression>of(ceylonLiteral(module.getNameAsString()), ceylonLiteral(module.getVersion())));
		}
		// make sure we handle missing modules gracefully
		String version = module.getVersion();
		return makeMetamodelInvocation("checkModule", List.of(call, ceylonLiteral(module.getNameAsString()),
				version == null ? makeNull() : ceylonLiteral(version)), null);
	}
	...
	private JCExpression getClosedTypesSequential(java.util.List<Type> typeModels) {
		...
		Type elementType = typeFact().getMetamodelTypeDeclaration().appliedType(null,
				Arrays.asList(typeFact().getAnythingType()));
	...
	JCExpression makeTypeLiteralCall(Type type, boolean addCast, Type exprType) {
		...
		if (addCast) {
			...
			exprType = exprType.resolveAliases();
			if (!exprType.isUnion() && !exprType.isExactlyNothing()
	...
	public JCTree transform(Tree.TypeLiteral expr) {
		...
		if (expr.getWantsDeclaration()) {
			...
			} else if (dec instanceof ClassOrInterface || dec instanceof TypeAlias) {
				// use the generated class to get to the declaration literal
				return make().TypeCast(makeJavaType(exprType, JT_NO_PRIMITIVES),
			...
			if (coerced) {
				if (!typeFact().isJavaObjectArrayType(type) || type.getTypeArgumentList().get(0).isClassOrInterface()) {
	 ...
	 */
	JCExpression makeTypeParameterDeclaration(Tree.TypeLiteral expr, TypeParameter declaration) {
		Scope container = declaration.getContainer();
		if (container instanceof Declaration) {
			...
			if (containerDeclaration instanceof ClassOrInterface || containerDeclaration instanceof TypeAlias) {
				JCExpression metamodelCast = makeJavaType(
						typeFact().getLanguageModuleDeclarationTypeDeclaration("GenericDeclaration").getType(),
						JT_NO_PRIMITIVES);
			...
			// now it must be a ClassOrInterfaceDeclaration or a FunctionDeclaration, both
			// of which have the method we need
			return at(expr).Apply(null, makeSelect(containerExpr, "getTypeParameterDeclaration"),
	...
	public JCExpression transformStringExpression(Tree.StringTemplate expr) {
		...
		builder = make().NewClass(null, null, naming.makeFQIdent("java", "lang", "StringBuilder"),
		...
		for (int ii = 0; ii < literals.size(); ii += 1) {
			...
			if (isCeylonBasicType(expression.getTypeModel()) && expression.getUnboxed()) {// TODO: Test should be erases
																							// to String, long, int,
																							// boolean, char, byte,
																							// float, double
				// If erases to a Java primitive just call append, don't box it just to call
				// format.
				String method = isCeylonCharacter(expression.getTypeModel()) ? "appendCodePoint" : "append";
				builder = make().Apply(null, makeSelect(builder, method),
						List.<JCExpression>of(transformExpression(expression, BoxingStrategy.UNBOXED, null)));
			} else {
				JCMethodInvocation formatted = make().Apply(null,
						makeSelect(transformExpression(expression), "toString"), List.<JCExpression>nil());
	...
	private JCExpression sequentialEmptiness(JCExpression sequential, Type expectedType, Type sequentialType) {
		int flags = 0;
		// make sure we detect that we're downcasting a sequential into a sequence if we
		// know the comprehension is non-empty
		if (expectedType.getSupertype(typeFact().getSequenceDeclaration()) != null)
			flags = EXPR_DOWN_CAST;
		return applyErasureAndBoxing(sequential, sequentialType, false, true, BoxingStrategy.BOXED, expectedType,
				flags);
	}
	...
	private JCExpression makeTuple(Type tupleType, java.util.List<Tree.PositionalArgument> expressions) {
			...
			if (expr instanceof Tree.ListedArgument) {
				...
				if (sequentialSpreadType != null) {
					...
					if (typeFact().isIterableType(spreadType)) {
						...
						tail = transformExpression(spreadExpr.getExpression(), BoxingStrategy.BOXED,
								iterableSpreadType);
						...
						tail = transformExpression(spreadExpr.getExpression(), BoxingStrategy.BOXED,
								iterableSpreadType);
						tail = utilInvocation().toIterable(makeJavaType(iteratedType, JT_TYPE_ARGUMENT),
				...
				return makeErroneous(expr,
						"compiler bug: " + expr.getNodeType() + " is not a supported tuple argument");
			}
		}
		...
		if (!elems.isEmpty()) {
			...
			return make().TypeCast(typeExpr, make().Apply(List.<JCExpression>nil(),
					naming.makeQualIdent(make().QualIdent(syms().ceylonTupleType.tsym), "instance"), args));
		} else {
			return tail;
		}
	}
	...
	public JCTree transform(Tree.Outer expr) {
		...
		Type outerClass = org.eclipse.ceylon.model.typechecker.model.ModelUtil
				.getOuterClassOrInterface(expr.getScope());
	...
	public JCExpression transform(Tree.NotOp op) {
		// No need for an erasure cast since Term must be Boolean and we never need to
		// erase that
	...
	public JCExpression transform(Tree.IsOp op) {
		// make sure we do not insert null checks if we're going to allow testing for
		// null
		Type expectedType = getOptionalTypeForInteropIfAllowed(op.getType().getTypeModel(), op.getTerm().getTypeModel(),
				op.getTerm());
		...
		at(op);
		Naming.SyntheticName varName = naming.temp();
		JCExpression test = makeOptimizedTypeTest(null, varName, op.getType().getTypeModel(),
				op.getTerm().getTypeModel());
	...
	public JCTree transform(Tree.Exists op) {
		// for the purpose of checking if something is null, we need it boxed and
		// optional, otherwise
		// for some Java calls if we consider it non-optional we will get an unwanted
		// null check
	...
	public BinOpTransformation transformNotEqualNeedsNegating(Tree.NotEqualOp op) {
		...
		// operator.getBinOpOptimisationStrategy(op, op.getLeftTerm(),
		// op.getRightTerm(), this);
		...
		// optimisationStrategy.getBoxingStrategy(), null, EXPR_WIDEN_PRIM);
		// we don't care about the right erased type, since equals() is on Object
		return transformOverridableBinaryOperator(op, operator, op.getLeftTerm().getTypeModel(),
				op.getRightTerm().getTypeModel());
	}
	...
	public JCExpression transform(Tree.SegmentOp op) {
		...
		JCExpression lengthExpr = transformExpression(op.getRightTerm(), BoxingStrategy.UNBOXED,
	...
	public JCExpression transform(Tree.EntryOp op) {
		...
		return at(op).NewClass(null, null, typeExpr,
				List.<JCExpression>of(makeReifiedTypeArgument(leftType), makeReifiedTypeArgument(rightType), key, elem),
				null);
	}
	...
	public JCExpression transform(Tree.DefaultOp op) {
		...
		JCExpression left = transformExpression(op.getLeftTerm(), BoxingStrategy.BOXED,
				typeFact().getOptionalType(typeModel));
	...
	public JCTree transform(Tree.ThenOp op) {
		...
		JCExpression right = transformExpression(op.getRightTerm(), CodegenUtil.getBoxingStrategy(op),
				op.getTypeModel());
	...
	public JCTree transform(Tree.InOp op) {
		...
		JCExpression right = transformExpression(op.getRightTerm(), BoxingStrategy.BOXED,
				op.getRightTerm().getTypeModel().getSupertype(typeFact().getCategoryDeclaration()));
		Naming.SyntheticName varName = naming.temp();
		JCExpression varIdent = varName.makeIdent();
	...
	protected JCTree makeOptimizedInIntegerOrCharacterMeasure(Tree.InOp op,
			org.eclipse.ceylon.langtools.tools.javac.code.Type ceylonType,
			org.eclipse.ceylon.langtools.tools.javac.code.Type javaType) {
		...
		JCExpression y = transformExpression(rangeOp.getLeftTerm(), BoxingStrategy.UNBOXED,
				rangeOp.getLeftTerm().getTypeModel());
		JCExpression z = transformExpression(rangeOp.getRightTerm(), BoxingStrategy.UNBOXED,
				rangeOp.getRightTerm().getTypeModel());
		JCExpression w = make().Apply(null, naming.makeSelect(make().QualIdent(ceylonType.tsym), "offset"),
				List.<JCExpression>of(xName.makeIdent(), yName.makeIdent()));
		return make().LetExpr(
				List.<JCStatement>of(makeVar(xName, make().Type(javaType), x), makeVar(yName, make().Type(javaType), y),
						makeVar(zName, make().Type(syms().longType), z),
						makeVar(wName, make().Type(syms().longType), w)),
				make().Binary(JCTree.Tag.AND, make().Binary(JCTree.Tag.GT, zName.makeIdent(), make().Literal(0L)),
	...
	protected JCTree makeOptimizedInIntegerRange(Tree.InOp op,
			org.eclipse.ceylon.langtools.tools.javac.code.Type type) {
		// x in y..z with x, y, z all Integer
		...
		JCExpression x = transformExpression(op.getLeftTerm(), BoxingStrategy.UNBOXED, typeFact().getObjectType());
		JCExpression first = transformExpression(rangeOp.getLeftTerm(), BoxingStrategy.UNBOXED,
				rangeOp.getLeftTerm().getTypeModel());
		JCExpression last = transformExpression(rangeOp.getRightTerm(), BoxingStrategy.UNBOXED,
				rangeOp.getRightTerm().getTypeModel());
		SyntheticName xName = naming.temp("x");
		...
		return make().LetExpr(
				List.<JCStatement>of(makeVar(xName, make().Type(type), x), makeVar(firstName, make().Type(type), first),
						makeVar(lastName, make().Type(type), last),
						makeVar(recursiveName, make().Type(syms().booleanType), make().Binary(JCTree.Tag.AND,
								make().Binary(JCTree.Tag.GT, firstName.makeIdent(),
										make().Binary(JCTree.Tag.PLUS, firstName.makeIdent(), make().Literal(1L))),
								make().Binary(JCTree.Tag.GT,
										make().Binary(JCTree.Tag.MINUS, lastName.makeIdent(),
												make().Literal(1L)),
										lastName.makeIdent())))),
				make().Conditional(recursiveName.makeIdent(),
						// x.offset(first) <= last.offset(first)
						make().Binary(JCTree.Tag.LE,
								make().Apply(null, naming.makeSelect(make().QualIdent(ceylonType.tsym), "offset"),
										List.<JCExpression>of(xName.makeIdent(), firstName.makeIdent())),
								make().Apply(null, naming.makeSelect(make().QualIdent(ceylonType.tsym), "offset"),
	...
	protected JCTree makeOptimizedInCharacterRange(Tree.InOp op) {
		...
		return make()
				.LetExpr(
						List.<JCStatement>of(makeVar(xName, make().Type(type), x),
								...
								makeVar(recursiveName, make().Type(syms().booleanType), make().Binary(JCTree.Tag.AND,
										make().Binary(JCTree.Tag.GT, firstName.makeIdent(), make().Apply(null,
												naming.makeSelect(make().QualIdent(ceylonType.tsym), "getSuccessor"),
												List.<JCExpression>of(firstName.makeIdent()))),
										make().Binary(JCTree.Tag.GT, make().Apply(null,
												naming.makeSelect(make().QualIdent(ceylonType.tsym), "getPredecessor"),
												List.<JCExpression>of(lastName.makeIdent())), lastName.makeIdent())))),
						make().Conditional(make().Binary(JCTree.Tag.LT, firstName.makeIdent(), lastName.makeIdent()),
								make().Binary(JCTree.Tag.AND,
										make().Binary(JCTree.Tag.LE, xName.makeIdent(), lastName.makeIdent()),
										make().Binary(JCTree.Tag.GE, xName.makeIdent(), firstName.makeIdent())),
								make().Binary(JCTree.Tag.AND,
										make().Binary(JCTree.Tag.GE, xName.makeIdent(), lastName.makeIdent()),
										make().Binary(JCTree.Tag.LE, xName.makeIdent(), firstName.makeIdent()))));
	}
	...
	private JCExpression transformLogicalOp(Node op, OperatorTranslation operator, JCExpression left,
			Tree.Term rightTerm) {
		// Both terms are Booleans and can't be erased to anything
	...
	public JCExpression transform(Tree.IdenticalOp op) {
		// The only thing which might be unboxed is boolean, and we can follow the rules
		// of == for optimising it,
		// which are simple and require that both types be booleans to be unboxed,
		// otherwise they must be boxed
		OptimisationStrategy optimisationStrategy = OperatorTranslation.BINARY_EQUAL.getBinOpOptimisationStrategy(op,
				op.getLeftTerm(), op.getRightTerm(), this);
	...
	public JCExpression transform(Tree.WithinOp op) {
		...
		List<JCStatement> vars = List
				.<JCStatement>of(makeVar(middleName, within.makeMiddleType(), within.makeMiddle()));
	...
	class WithinTransformation {
		...
		public WithinTransformation(Tree.WithinOp op) {
			...
			this.opt = getWithinOptimization(middleTerm, middleType, lowerBound, lowerType, upperBound, upperType);
		}
		...
		protected OperatorTranslation getOperatorTranslation(Tree.Bound lowerBound) {
			return lowerBound instanceof Tree.OpenBound ? OperatorTranslation.BINARY_SMALLER
					: OperatorTranslation.BINARY_SMALL_AS;
		}

		private JCExpression transformWithin(Tree.WithinOp op, OptimisationStrategy opt, Tree.Term middleTerm,
				SyntheticName middleName, Type middleType, JCExpression left, Tree.Bound lowerBound, Type lowerType,
				JCExpression right, Tree.Bound upperBound, Type upperType) {

			JCExpression lower = transformBound(lowerBound, middleName.makeIdent(), middleType, left, lowerType,
					getOperatorTranslation(lowerBound), opt, middleTerm, false);
			JCExpression upper = transformBound(upperBound, middleName.makeIdent(), middleType, right, upperType,
					getOperatorTranslation(upperBound), opt, middleTerm, true);
			at(op);
			...
			JCExpression andExpr = transformOverridableBinaryOperator(op, andOp, optimisationStrategy, lower, upper,
					null, null, op.getTypeModel()).build();
			return andExpr;
		}

		protected OptimisationStrategy getWithinOptimization(Tree.Term middleTerm, Type middleType,
				Tree.Bound lowerBound, Type lowerType, Tree.Bound upperBound, Type upperType) {
			...
			boolean optimizeLower = lowerOp.isTermOptimisable(lowerTerm, lowerType,
					ExpressionTransformer.this) == OptimisationStrategy.OPTIMISE
					|| lowerOp.isTermOptimisable(middleTerm, middleType,
							ExpressionTransformer.this) == OptimisationStrategy.OPTIMISE;
			boolean optimizeUpper = upperOp.isTermOptimisable(middleTerm, middleType,
					ExpressionTransformer.this) == OptimisationStrategy.OPTIMISE
					|| upperOp.isTermOptimisable(upperTerm, upperType,
							ExpressionTransformer.this) == OptimisationStrategy.OPTIMISE;
			if ((lowerType.isExactly(middleType) && middleType.isExactly(upperType) && (optimizeLower || optimizeUpper))// if
																														// all
																														// same
																														// type
																														// and
		...
		private JCExpression transformBound(Tree.Bound bound, JCExpression endpoint1, Type middleType,
				JCExpression endpoint2, Type otherType, final OperatorTranslation operator,
				final OptimisationStrategy optimisationStrategy, Tree.Term middleTerm, boolean isUpper) {
			...
			at(bound);
			return transformOverridableBinaryOperator(middleTerm, operator, optimisationStrategy, left, right, null,
					leftType, null, bound.getUnit().getBooleanType()).build();
		}

		public JCExpression build() {
			JCExpression andExpr = transformWithin(op, opt, middleTerm, middleName, middleType, left, lowerBound,
					lowerType, right, upperBound, upperType);
			return andExpr;
		}
	}
	...
	public JCExpression transform(Tree.ScaleOp op) {
		...
		JCVariableDecl scaleable = makeVar(scaleableName, makeJavaType(scalableTermType, JT_NO_PRIMITIVES),
		...
		at(op);
		return make()
				.LetExpr(List.<JCStatement>of(scale, scaleable),
						transformOverridableBinaryOperator(op, operator, OptimisationStrategy.NONE,
								scaleableName.makeIdent(), scaleName.makeIdent(), null, null, op.getTypeModel())
										.build());
	}
	...
	/**
	 * Returns the literal value of the power in the given power expression, or null
	 * if the power is not an integer literal (or negation of an integer literal)
	 * 
	 * @throws ErroneousException
	 */
	...
	private BinOpTransformation transformOverridableBinaryOperator(Tree.BinaryOperatorExpression op,
			Interface compoundType, int typeArgumentToUse) {
		Type leftType;
		Type rightType;
		// we do have a special case which is when the LHS is Float and RHS is Integer
		// and the typechecker allowed coercion
		...
		if (getSupertype(leftTerm, typeFact().getFloatDeclaration()) != null
			...
			// Also keep the LHS type as Float since it's final and the special methods
			// wouldn't be found in any supertype of compountType
			leftType = typeFact().getFloatType();
			// keep the RHS type then, since floats are not integers, the whole thing is
			// resolved in the Java impl of Float with
			...
			// the right type always only depends on the LHS so let's not try to find it on
			// the right side because it may
			...
			if (leftType.isInteger() && rightType.isInteger() && rightTerm.getTypeModel().isFloat()) {
	...
	BinOpTransformation transformOverridableBinaryOperator(Tree.BinaryOperatorExpression op, Type leftType,
			Type rightType) {
		OperatorTranslation operator = Operators.getOperator(op);
	...
	protected BinOpTransformation transformOverridableBinaryOperator(Tree.BinaryOperatorExpression op,
		...
		OptimisationStrategy optimisationStrategy = operator.getBinOpOptimisationStrategy(op, op.getLeftTerm(),
				leftType, op.getRightTerm(), rightType, this);
		...
		JCExpression right = transformExpression(op.getRightTerm(), optimisationStrategy.getBoxingStrategy(), rightType,
				EXPR_WIDEN_PRIM);
		return transformOverridableBinaryOperator(op, operator, optimisationStrategy, left, right, op.getLeftTerm(),
				op.getRightTerm(), op.getTypeModel());
	}

	private JCExpression transformOverridableBinaryOperator(Tree.Term opExpr, Tree.Term leftTerm, Tree.Term rightTerm,
			Type rightType, OperatorTranslation operator, OptimisationStrategy optimisationStrategy, JCExpression left,
			Type expectedType) {
		JCExpression right = transformExpression(rightTerm, optimisationStrategy.getBoxingStrategy(), rightType,
				EXPR_WIDEN_PRIM);
		return transformOverridableBinaryOperator(opExpr, operator, optimisationStrategy, left, right, leftTerm,
				rightTerm, expectedType).build();
	}

	private BinOpTransformation transformOverridableBinaryOperator(Tree.Term opExpr,
			OperatorTranslation originalOperator, OptimisationStrategy optimisationStrategy, JCExpression left,
			JCExpression right, Tree.Term leftTerm, Tree.Term rightTerm, Type expectedType) {
		return transformOverridableBinaryOperator(opExpr, originalOperator, optimisationStrategy, left, right, leftTerm,
				leftTerm != null ? leftTerm.getTypeModel() : null, rightTerm, expectedType);
	}
	...
	class BinOpTransformation {
		...
		public JCExpression build() {
			...
			if (operator == OperatorTranslation.BINARY_UNION || operator == OperatorTranslation.BINARY_INTERSECTION
			...
			if (optimisationStrategy.useValueTypeMethod()) {
				...
				result = at(opExpr).Apply(typeArgs,
						naming.makeQualIdent(makeJavaType(leftType, flags), operator.getCeylonValueTypeMethodName()),
						args.prepend(left));
			} else {
				if ((operator == OperatorTranslation.BINARY_COMPARE || operator == OperatorTranslation.BINARY_SMALL_AS
						|| operator == OperatorTranslation.BINARY_LARGE_AS
						|| operator == OperatorTranslation.BINARY_SMALLER
						|| operator == OperatorTranslation.BINARY_LARGER) && willEraseToObject(leftType)) {
					left = make().TypeCast(makeJavaType(typeFact().getComparableDeclaration().getType(), JT_RAW), left);
					args = List.<JCExpression>of(make()
							.TypeCast(makeJavaType(typeFact().getComparableDeclaration().getType(), JT_RAW), right));
				}
	...
	private BinOpTransformation transformOverridableBinaryOperator(Tree.Term opExpr, OperatorTranslation operator,
			OptimisationStrategy optimisationStrategy, JCExpression left, JCExpression right, Tree.Term leftTerm,
			Type leftType, Tree.Term rightTerm, Type expectedType) {
	...
	public JCExpression transform(final Tree.ArithmeticAssignmentOp op) {
		...
		if (operator == null) {
			return makeErroneous(op,
					"compiler bug: " + op.getNodeType() + " is not a supported arithmetic assignment operator");
		}
				...
				new AssignAndReturnOperationFactory() {
					...
					public JCExpression getNewValue(JCExpression previousValue) {
						...
						return transformOverridableBinaryOperator(op, leftTerm, rightTerm, rightType,
								...
								boxResult ? OptimisationStrategy.NONE : OptimisationStrategy.OPTIMISE, previousValue,
								op.getTypeModel());
					}
				});
	}
	...
	public JCExpression transform(final Tree.BitwiseAssignmentOp op) {
		...
		if (operator == null) {
			return makeErroneous(op,
					"compiler bug: " + op.getNodeType() + " is not a supported bitwise assignment operator");
		}
	...
	public JCExpression transform(final Tree.LogicalAssignmentOp op) {
		...
		if (operator == null) {
			return makeErroneous(op,
					"compiler bug: " + op.getNodeType() + " is not a supported logical assignment operator");
		}
		...
		return transformAssignAndReturnOperation(op, leftTerm, false, valueType, valueType,
				new AssignAndReturnOperationFactory() {
					@Override
					public JCExpression getNewValue(JCExpression previousValue) {
						// make this call: previousValue OP RHS
						return transformLogicalOp(op, operator.binaryOperator, previousValue, rightTerm);
					}
				});
	}

	private JCExpression optimiseAssignmentOperator(final Tree.AssignmentOp op,
			final AssignmentOperatorTranslation operator) {
	...
	public JCExpression transform(Tree.PostfixOperatorExpression expr) {
		...
		if (term instanceof Tree.BaseMemberExpression
				// special case for java statics Foo.attr where Foo does not need to be
				// evaluated
			...
			getter = applyErasureAndBoxing(getter, term, boxResult ? BoxingStrategy.BOXED : BoxingStrategy.UNBOXED,
					returnType);
			...
			if (canOptimise) {
				...
				successor = make().Binary(
						operator == OperatorTranslation.UNARY_POSTFIX_INCREMENT ? JCTree.Tag.PLUS : JCTree.Tag.MINUS,
						make().Ident(varName), makeInteger(1));
				successor = unAutoPromote(successor, returnType, expr.getSmall());
			} else {
				successor = make().Apply(null, makeSelect(make().Ident(varName), operator.getCeylonMethodName()),
						List.<JCExpression>nil());
				// make sure the result is boxed if necessary, the result of
				// successor/predecessor is always boxed
				successor = boxUnboxIfNecessary(successor, true, term.getTypeModel(),
						CodegenUtil.getBoxingStrategy(term));
			}
			...
			// e.attr++
			// (let $tmpE = e, $tmpV = $tmpE.attr; $tmpE.attr = $tmpV.getSuccessor();
			// $tmpV;)
			...
			if (canOptimise) {
				...
				successor = make().Apply(null, makeSelect(make().Ident(varVName), operator.getCeylonMethodName()),
						List.<JCExpression>nil());
				// make sure the result is boxed if necessary, the result of
				// successor/predecessor is always boxed
				successor = boxUnboxIfNecessary(successor, true, term.getTypeModel(),
		...
		// (let $tmpA = a, $tmpI = i, $tmpV = $tmpA.item($tmpI); $tmpA.setItem($tmpI,
		// $tmpV.getSuccessor()); $tmpV;)
	...
	public JCExpression transform(final Tree.PrefixOperatorExpression expr) {
		...
		return transformAssignAndReturnOperation(expr, term, !canOptimise, term.getTypeModel(), returnType,
				new AssignAndReturnOperationFactory() {
					@Override
					public JCExpression getNewValue(JCExpression previousValue) {
						// use +1/-1 if we can optimise a bit
						if (canOptimise) {
							JCExpression ret = make()
									.Binary(operator == OperatorTranslation.UNARY_PREFIX_INCREMENT ? JCTree.Tag.PLUS
											: JCTree.Tag.MINUS, previousValue, makeInteger(1));
							ret = unAutoPromote(ret, returnType, expr.getSmall());
							return ret;
						}
						// make this call: previousValue.getSuccessor() or
						// previousValue.getPredecessor()
						return make().Apply(null, makeSelect(previousValue, operator.getCeylonMethodName()),
								List.<JCExpression>nil());
					}
				});
	}
	...
	private JCExpression transformAssignAndReturnOperation(Node operator, Tree.Term term, boolean boxResult,
			Type valueType, Type returnType, AssignAndReturnOperationFactory factory) {
		...
		if (term instanceof Tree.BaseMemberExpression || (term instanceof Tree.IndexExpression)
		// special case for java statics Foo.attr where Foo does not need to be
		// evaluated
				|| (term instanceof Tree.QualifiedMemberExpression
						&& ((Tree.QualifiedMemberExpression) term).getStaticMethodReference())) {
			...
			JCExpression attrType = makeJavaType(returnType, boxResult ? JT_NO_PRIMITIVES : 0);
			Name varVName = naming.tempName("opV");
			JCExpression getter = transformMemberExpression(qualified,
					isSuper ? transformSuper(qualified) : make().Ident(varEName), null);
			// make sure we box the results if necessary
			getter = applyErasureAndBoxing(getter, term, boxResult ? BoxingStrategy.BOXED : BoxingStrategy.UNBOXED,
					valueType);
			JCExpression newValue = factory.getNewValue(getter);
			...
			return makeErroneous(operator,
					"compiler bug: " + term.getNodeType() + " is not a supported assign and return operator");
		}
		// OP(e?.attr) is probably not legal
		// OP(a[i]) is not for M1 but will be:
		// (let $tmpA = a, $tmpI = i, $tmpV = OP($tmpA.item($tmpI));
		// $tmpA.setItem($tmpI, $tmpV); $tmpV;)
	...
	public JCExpression transform(Tree.Parameter param) {
		// Transform the expression marking that we're inside a defaulted parameter for
		// $this-handling
		// needDollarThis = true;
		JCExpression expr;
		at(param);
		...
		if (Strategy.hasDefaultParameterValueMethod(param.getParameterModel())) {
			...
			if (param instanceof Tree.FunctionalParameterDeclaration) {
				...
				expr = CallableBuilder.anonymous(gen(), param,
						(Function) fpTree.getTypedDeclaration().getDeclarationModel(), lazy.getExpression(),
						((Tree.MethodDeclaration) fpTree.getTypedDeclaration()).getParameterLists(),
						getTypeForFunctionalParameter(fp), true).build();
			} else {
	...
	protected final JCExpression transformArg(SimpleInvocation invocation, int argIndex) {
		...
		if (invocation.hasParameter(argIndex)) {
			...
			if (invocation.isParameterSequenced(argIndex)
				...
				if (!invocation.isArgumentSpread(argIndex)) {
				...
				} else if (invocation.getArgumentType(argIndex)
						.getSupertype(typeFact().getSequentialDeclaration()) == null) {
					...
					type = org.eclipse.ceylon.model.typechecker.model.ModelUtil.appliedType(
							typeFact().getIterableDeclaration(), typeFact().getIteratedType(type),
							typeFact().getIteratedAbsentType(type));
				}
			}
			...
			if (coerced && term instanceof Tree.InvocationExpression && isFunctionalResult(term.getTypeModel())
	...
	private void appendImplicitArguments(Invocation invocation, TransformedInvocationPrimary transformedPrimary,
		...
		if (!(primaryDeclaration instanceof Value)) {
			if (!(primaryDeclaration instanceof Class) || !isJavaArray(((Class) primaryDeclaration).getType())) {
		...
		if (!(primary instanceof Tree.BaseTypeExpression) && !(primary instanceof Tree.QualifiedTypeExpression)
				...
				&& Invocation.onValueType(this, primary, primaryDeclaration) && transformedPrimary != null) {
	...
	private List<ExpressionAndType> transformArgumentsForSimpleInvocation(SimpleInvocation invocation,
		...
		if (!(invocation instanceof SuperInvocation) || !((SuperInvocation) invocation).isDelegationDelegation()) {
			...
			for (int argIndex = 0; argIndex < numArguments; argIndex++) {
				...
				// for Java methods of variadic primitives, it's better to wrap them ourselves
				// into an array
				// to avoid ambiguity of foo(1,2) for foo(int...) and foo(Object...) methods
				if (!wrapIntoArray && invocation.isParameterJavaVariadic(argIndex)
						&& boxingStrategy == BoxingStrategy.UNBOXED
						&& willEraseToPrimitive(typeFact().getDefiniteType(parameterType)) && !invocation.isSpread())
					wrapIntoArray = true;
				...
				if (invocation.isArgumentSpread(argIndex)) {
					...
					if (invocation.isJavaVariadicMethod()) {
						...
						exprAndType = transformSpreadArgument(invocation, numArguments, argIndex, boxingStrategy,
								parameterType);
						...
						if (argType.getSupertype(typeFact().getSequentialDeclaration()) != null) {
							...
							if (typeFact().isJavaPrimitiveArrayType(argType)) {
								...
								iterable = utilInvocation().toIterable(makeJavaType(elementType, JT_TYPE_ARGUMENT),
										makeReifiedTypeArgument(elementType), exprAndType.expression);
							}
							exprAndType = new ExpressionAndType(
									make().Apply(null, makeSelect(iterable, "sequence"), List.<JCExpression>nil()),
							...
							JCExpression iterable = utilInvocation().toIterable(
									makeJavaType(elementType, JT_TYPE_ARGUMENT), makeReifiedTypeArgument(elementType),
							...
							exprAndType = new ExpressionAndType(
									makeErroneous(invocation.getNode(), "compiler bug: unexpected spread argument"),
									makeErroneous(invocation.getNode(), "compiler bug: unexpected spread argument"));
						}
					}
					...
					exprAndType = transformArgument(invocation, argIndex, boxingStrategy);
					// Callable has a variadic 1-param method that if you invoke it with a Java
					// Object[] will confuse javac and give
					// preference to the variadic method instead of the $call$(Object) one, so we
					// force a cast to Object to resolve it
					...
					if (numArguments == 1 && invocation.isIndirect()) {
						...
						if (isJavaObjectArray(argumentType) || isNull(argumentType)) {
							exprAndType = new ExpressionAndType(
									make().TypeCast(makeJavaType(typeFact().getObjectType()), exprAndType.expression),
									exprAndType.type);
						}
					} else if (invocation.isParameterJavaVariadic(argIndex) && !invocation.isSpread()) {
						// in fact, the very same problem happens when passing null or object arrays to
						// a java variadic method
						Type argumentType = invocation.getArgumentType(argIndex);
						if (isJavaObjectArray(argumentType) || isNull(argumentType)) {
							// remove any ambiguity
							exprAndType = new ExpressionAndType(
									make().TypeCast(makeJavaType(parameterType), exprAndType.expression),
									exprAndType.type);
						}
					}
				} else {
			...
			if (invocation.isIndirect() && invocation.getNumParameters() > numArguments
			...
			for (Parameter p : constructorDelegation.getConstructor().getParameterList().getParameters()) {
				result = result.append(new ExpressionAndType(
						naming.makeName(p.getModel(), Naming.NA_IDENT | Naming.NA_ALIASED), null));
			}
		}
		...
		if (superConstructor == null && concreteDelegation) {
			...
			result = result.prepend(new ExpressionAndType(naming.makeNamedConstructorName(delegateTo, true),
					naming.makeNamedConstructorType(delegateTo, true)));
		} else if (superConstructor != null && constructorDelegation != null
				&& constructorDelegation.isSelfDelegation()) {
			result = result.prepend(new ExpressionAndType(
					naming.makeNamedConstructorName(constructorDelegation.getExtendingConstructor(),
							concreteDelegation),
					naming.makeNamedConstructorType(constructorDelegation.getExtendingConstructor(),
							concreteDelegation)));
		} else if (superConstructor != null && !Decl.isDefaultConstructor(superConstructor)
				&& !Decl.isJavaArrayWith(superConstructor) && (!(qmep instanceof Tree.QualifiedTypeExpression)
	...
	private ExpressionAndType transformVariadicArgument(SimpleInvocation invocation, int numArguments, int argIndex,
			Type parameterType) {
		ExpressionAndType exprAndType;
	...
	private ExpressionAndType transformSpreadArgument(SimpleInvocation invocation, int numArguments, int argIndex,
			BoxingStrategy boxingStrategy, Type parameterType) {
		ExpressionAndType exprAndType;
		final Type iteratedType = typeFact().getIteratedType(parameterType);
		final JCExpression expr;
		final JCExpression type;
		// optimise "*javaArray.iterable" into "javaArray" for java variadic parameters,
		// since we can pass them just along
		if (invocation.isJavaVariadicMethod() && numArguments == argIndex + 1
			...
			if (argument instanceof Tree.QualifiedMemberExpression) {
				...
				if ("iterable".equals(qualifiedMemberArgument.getIdentifier().getText())
					...
					// we don't care at all about unboxing or casting since we can't be dealing with
					// boxing
					// and we generate our own cast, at least for non-primitive arrays where it may
					// be ambiguous,
					// we could avoid the cast for non-type-parameter and non-Object arrays, but
					...
					if (isJavaObjectArray(qualifiedMemberArgument.getPrimary().getTypeModel())) {
						expr = make().TypeCast(makeJavaType(qualifiedMemberArgument.getPrimary().getTypeModel()),
								primary);
					} else {
						expr = primary;
					}
		...
		for (int ii = argIndex; ii < numArguments; ii++) {
			...
			if (ii < numArguments - 1) {
				...
				if (typeFact().isJavaArrayType(argType)) {
					...
					argExpr = make().Apply(null, naming.makeQuotedQualIdentFromString(methodName),
							List.<JCExpression>of(argExpr));
					...
					argExpr = utilInvocation().toIterable(makeJavaType(iteratedType, JT_TYPE_ARGUMENT),
		...
		if (invocation.isJavaVariadicMethod()) {
			...
			JCExpression sequentialExpr = utilInvocation().sequentialInstance(typeExpr,
					makeReifiedTypeArgument(iteratedType), x.head, x.tail);
			if (invocation.isParameterVariadicPlus(argIndex)) {
	...
	private List<ExpressionAndType> transformSpreadTupleArgument(SimpleInvocation invocation, CallBuilder callBuilder,
		...
		if (typeFact().isIterableType(tupleArgument.getTypeModel())) {
			tupleExpr = make().TypeCast(makeJavaType(typeFact().getSequentialDeclaration().getType(), JT_RAW),
					tupleExpr);
			...
			tupleExpr = utilInvocation().toIterable(makeJavaType(iteratedType, JT_TYPE_ARGUMENT),
		...
		callBuilder.voidMethod(invocation.getReturnType() == null
				|| Decl.isUnboxedVoid(invocation.getPrimaryDeclaration()) || isWithinSuperInvocation());

		/*
		 * Cases: [] -> () => nothing [] -> (Integer=) => nothing [] -> (Integer*) =>
		 * nothing [Integer] -> (Integer) => extract [Integer] -> (Integer=) => extract
		 * [Integer] -> (Integer*) => pass the tuple as-is [Integer*] -> (Integer*) =>
		 * pass the tuple as-is [Integer+] -> (Integer*) => pass the tuple as-is
		 * [Integer] -> (Integer, Integer*) => extract and drop the tuple
		 * [Integer,Integer] -> (Integer, Integer) => extract [Integer,Integer] ->
		 * (Integer=, Integer=) => extract [Integer,Integer] -> (Integer, Integer*) =>
		 * extract and pass the tuple rest [Integer,Integer*] -> (Integer, Integer*) =>
		 * extract and pass the tuple rest [Integer,Integer+] -> (Integer, Integer*) =>
		 * extract and pass the tuple rest
		 */
		...
		// we extract from the tuple not more than we have tuple members, but even less
		// than that if we don't
		// have enough parameters to put them in
		final int argumentsToExtract = Math.min(argIndex + minimumTupleArguments,
				variadic ? maxParameters - 1 : maxParameters);
		for (; spreadArgIndex < argumentsToExtract; spreadArgIndex++) {
			...
			JCExpression tupleIndex = boxType(make().Literal((long) spreadArgIndex - argIndex),
					typeFact().getIntegerType());
			JCExpression tupleElement = make().Apply(null, naming.makeQualIdent(tupleAlias.makeIdent(), "get"),
					List.<JCExpression>of(tupleIndex));

			tupleElement = applyErasureAndBoxing(tupleElement, typeFact().getAnythingType(), true, boxingStrategy,
					paramType);
			JCExpression argType = makeJavaType(paramType,
					boxingStrategy == BoxingStrategy.BOXED ? JT_NO_PRIMITIVES : 0);
			result = result.append(new ExpressionAndType(tupleElement, argType));
		}
		// if we're variadic AND
		// - the tuple is unbounded (which means we must have an unknown number of
		// elements left to pass)
		...
		if (variadic && (tupleUnbounded || argumentsToExtract < (minimumTupleArguments + argIndex))) {
			...
			// argIndex = 1, tuple = [Integer], params = [Integer, Integer*], spreadArgIndex
			// = 1 => no span
			// argIndex = 0, tuple = [Integer+], params = [Integer, Integer*],
			// spreadArgIndex = 1 => spanFrom(1)
			...
			tupleElement = applyErasureAndBoxing(tupleElement, typeFact().getAnythingDeclaration().getType(), true,
					boxingStrategy, paramType);
			JCExpression argType = makeJavaType(paramType,
					boxingStrategy == BoxingStrategy.BOXED ? JT_NO_PRIMITIVES : 0);

			JCExpression expr;
			if (invocation.isJavaVariadicMethod()) {
				// no need to handle leading arguments since that is handled by
				// transformSpreadArgument
				// if ever we have leading arguments we never end up in this method
				expr = sequenceToJavaArray(invocation, tupleElement, paramType, boxingStrategy, paramType,
						List.<JCExpression>nil());
			} else {
				expr = tupleElement;
			}
			result = result.append(new ExpressionAndType(expr, argType));
		} else if (variadic && invocation.isIndirect() && argumentsToExtract >= minimumTupleArguments
				&& !tupleUnbounded) {
			result = result.append(new ExpressionAndType(makeEmptyAsSequential(true),
					makeJavaType(typeFact().getSequenceType(typeFact().getAnythingDeclaration().getType()), JT_RAW)));
	...
	private ExpressionAndType transformArgument(SimpleInvocation invocation, int argIndex,
			BoxingStrategy boxingStrategy) {
		ExpressionAndType exprAndType;
		...
		if (argIndex == 0 && typeFact().isOptionalType(paramType)
				&& invocation.getArgumentType(argIndex).isSubtypeOf(typeFact().getNullType()) && ctedClass != null
				&& (ctedClass.hasConstructors() || ctedClass.isSerializable())) {
			// we've invoking the default constructor, whose first parameter has optional
			// type
			...
			// disambiguate
			expr = make().TypeCast(
					makeJavaType(paramType, boxingStrategy == BoxingStrategy.BOXED ? JT_NO_PRIMITIVES : 0), expr);
		}
		exprAndType = new ExpressionAndType(expr, type);
		return exprAndType;
	}
	...
	private List<ExpressionAndType> transformArgumentsForCallableSpecifier(CallableSpecifierInvocation invocation) {
		...
		for (Parameter parameter : invocation.getMethod().getFirstParameterList().getParameters()) {
			...
			arg = expressionGen().applyErasureAndBoxing(arg, exprType, !parameter.getModel().getUnboxed(),
	...
	public final JCExpression transformInvocation(final Invocation invocation) {
		...
		try {
			...
			if (invocation instanceof CallableSpecifierInvocation) {
				...
				JCExpression result = transformTermForInvocation(primary,
						new InvocationTermTransformer(invocation, callBuilder));
				return result;
	...
	protected JCExpression transformPositionalInvocationOrInstantiation(Invocation invocation, CallBuilder callBuilder,
		...
		if (invocation.handleBoxing)
			resultExpr = applyErasureAndBoxing(resultExpr, invocation.getReturnType(), invocation.erased,
					!invocation.unboxed, invocation.boxingStrategy, invocation.getReturnType(), 0);
		return resultExpr;
	}
		...
		if (Decl.isConstructorPrimary(invocation.getPrimary())) {
			...
			if (Strategy.generateInstantiator(ctor)) {
				...
				if (qte instanceof Tree.QualifiedMemberExpression
					...
					callBuilder.invoke(naming.makeInstantiatorMethodName(transformedPrimary.expr,
							ModelUtil.getConstructedClass(ctor)));
				}
			} else if (typeFact().isJavaArrayType(ModelUtil.getConstructedClass(ctor).getType())) {
				callBuilder.arrayWith(invocation.getReturnType().getQualifyingType(),
				...
				if (ModelUtil.getConstructedClass(invocation.getPrimaryDeclaration()).isMember()
						...
						&& !(((Tree.QualifiedMemberOrTypeExpression) invocation.getPrimary())
								.getPrimary() instanceof Tree.BaseTypeExpression)) {
					callBuilder.instantiate(new ExpressionAndType(transformedPrimary.expr, null),
							makeJavaType(invocation.getReturnType(),
									JT_CLASS_NEW | (transformedPrimary.expr == null ? 0 : JT_NON_QUALIFIED)));
				} else {
					callBuilder.instantiate(
							makeJavaType(invocation.getReturnType(), JT_CLASS_NEW)/* transformedPrimary.expr */);
				}
			}
		} else if (invocation.getQmePrimary() != null && isJavaArray(invocation.getQmePrimary().getTypeModel())
				&& transformedPrimary.selector != null
				&& (transformedPrimary.selector.equals("get") || transformedPrimary.selector.equals("set"))) {
			if (transformedPrimary.selector.equals("get"))
				...
				return makeErroneous(invocation.getNode(),
						"compiler bug: extraneous array selector: " + transformedPrimary.selector);
		} else if (invocation.isUnknownArguments()) {
			// if we have an unknown parameter list, like Callble<Ret,Args>, need to prepend
			// the callable
			// to the argument list, and invoke Util.apply
			// note that ATM the typechecker only allows a single argument to be passed in
			// spread form in this
			...
			callBuilder
					.invoke(make().Select(make().QualIdent(syms().ceylonUtilType.tsym), names().fromString("apply")));
		} else if (invocation.isOnValueType()) {
			JCExpression primTypeExpr = makeJavaType(invocation.getQmePrimary().getTypeModel(),
					JT_NO_PRIMITIVES | JT_VALUE_TYPE);
	...
	private JCExpression transformQualifiedInstantiation(Invocation invocation, CallBuilder callBuilder,
		...
		if (Decl.isJavaStaticOrInterfacePrimary(invocation.getPrimary())) {
			...
			if (Decl.isConstructorPrimary(invocation.getPrimary())) {
				 ...
				 * !(((Tree.QualifiedTypeExpression)invocation.getPrimary()).getPrimary()
				 * instanceof Tree.BaseTypeExpression)
				 */) {
					callBuilder.instantiate(new ExpressionAndType(transformedPrimary.expr, null),
							makeJavaType(invocation.getReturnType(),
									JT_CLASS_NEW | (transformedPrimary.expr == null ? 0 : JT_NON_QUALIFIED)));
				...
				if (qualifier == null) {
					...
					// Note: here we're not fully qualifying the class name because the JLS says
					// that if "new" is qualified the class name
					// is qualified relative to it
					type = makeJavaType(classType,
							AbstractTransformer.JT_CLASS_NEW | AbstractTransformer.JT_NON_QUALIFIED);
				}
			...
			callBuilder.invoke(naming.makeInstantiatorMethodName(transformedPrimary.expr,
					ModelUtil.getConstructedClass(declaration)));
		}
		JCExpression result = callBuilder.build();
	...
	private JCExpression transformBaseInstantiation(Invocation invocation, CallBuilder callBuilder,
		...
		if (Strategy.generateInstantiator(declaration)) {
			resultExpr = callBuilder.typeArguments(List.<JCExpression>nil())
					.invoke(naming.makeInstantiatorMethodName(transformedPrimary.expr, (Class) declaration)).build();
			if (Strategy.isInstantiatorUntyped(declaration)) {
			...
			if (isJavaArray(classType)) {
				JCExpression typeExpr = makeJavaType(classType,
						AbstractTransformer.JT_CLASS_NEW | AbstractTransformer.JT_RAW);
				...
				if (isJavaObjectArray(classType)) {
					...
					if (elementType.getDeclaration() instanceof ClassOrInterface || elementType.isNothing()) {
						if (!elementType.getTypeArgumentList().isEmpty())
							callBuilder.javaArrayInstanceNeedsCast(
									makeJavaType(classType, AbstractTransformer.JT_NO_PRIMITIVES));
					} else {
						// if it's an array of union, intersection or type param we need a runtime
						// allocation
	...
	private JCExpression transformCallableSpecifierInvocation(CallBuilder callBuilder,
			CallableSpecifierInvocation invocation) {
		at(invocation.getNode());
		JCExpression result = callBuilder
				.invoke(naming.makeQuotedQualIdent(invocation.getCallable(),
						Naming.getCallableMethodName(invocation.getMethod())))
				.argumentsAndTypes(transformArguments(invocation, null, callBuilder)).build();
		if (invocation.handleBoxing)
			result = applyErasureAndBoxing(result, invocation.getReturnType(), invocation.erased, !invocation.unboxed,
					invocation.boxingStrategy, invocation.getReturnType(), 0);
		return result;
	}
			...
			for (TypeParameter tp : tps) {
				...
				if (willEraseToObject(ta) || needsCastForBounds) {
					...
					if (hasDependentTypeParameters(tps, tp)
							// if we must use the bounds and we have more than one, we cannot use one to
							// satisfy them all
							// and we cannot represent the intersection type in Java so give up
							|| hasMultipleBounds
							// if we are going to use the first bound and it is self-dependent, we will make
							// it raw
						...
						// so at some point we'll have to introduce an intersection type AST node to
						// satisfy multiple bounds
	...
	protected JCExpression transformNamedArgumentInvocationOrInstantiation(NamedArgumentInvocation invocation,
			CallBuilder callBuilder, TransformedInvocationPrimary transformedPrimary) {
		JCExpression resultExpr = transformPositionalInvocationOrInstantiation(invocation, callBuilder,
				transformedPrimary);
		...
		if (invocation.getVars() != null && !invocation.getVars().isEmpty()) {
			if ((invocation.getReturnType() == null || Decl.isUnboxedVoid(invocation.getPrimaryDeclaration()))
				...
				// void methods get wrapped like (let $arg$1=expr, $arg$0=expr in call($arg$0,
				// $arg$1); null)
				resultExpr = make().LetExpr(invocation.getVars().append(make().Exec(resultExpr)).toList(), makeNull());
			} else {
				// all other methods like (let $arg$1=expr, $arg$0=expr in call($arg$0, $arg$1))
				resultExpr = make().LetExpr(invocation.getVars().toList(), resultExpr);
			}
		}
		return resultExpr;
	}
	...
	public void transformSuperInvocation(Tree.ExtendedType extendedType, ClassDefinitionBuilder classBuilder) {
		...
		if (extendedType.getInvocationExpression() != null
			...
			Declaration primaryDeclaration = ((Tree.MemberOrTypeExpression) extendedType.getInvocationExpression()
					.getPrimary()).getDeclaration();
			java.util.List<ParameterList> paramLists = ((Functional) primaryDeclaration).getParameterLists();
			if (paramLists.isEmpty()) {
				classBuilder.getInitBuilder()
						.delegateCall(at(extendedType).Exec(
								makeErroneous(extendedType, "compiler bug: missing parameter list in extends clause: "
										+ primaryDeclaration.getName() + " must be invoked")));
			} else {
				boolean prevFnCall = withinInvocation(true);
				try {
					JCStatement superExpr = transformConstructorDelegation(extendedType,
							new CtorDelegation(null, primaryDeclaration), extendedType.getInvocationExpression(),
							classBuilder, false);
	...
	/**
	 * Transform a delegated constructor call ({@code extends XXX()}) which may be
	 * either a superclass initializer/constructor or a same-class constructor.
	 * 
	 * @param extendedType
	 * @param delegation
	 *            The kind of delegation
	 * @param invocation
	 * @param classBuilder
	 * @return
	 */
	JCStatement transformConstructorDelegation(Node extendedType, CtorDelegation delegation,
			Tree.InvocationExpression invocation, ClassDefinitionBuilder classBuilder,
			boolean forDelegationConstructor) {
		...
		java.util.List<ParameterList> paramLists = ((Functional) primaryDeclaration).getParameterLists();
		if (paramLists.isEmpty()) {
			classBuilder.getInitBuilder().delegateCall(at(extendedType).Exec(makeErroneous(extendedType,
					"compiler bug: super class " + primaryDeclaration.getName() + " is missing parameter list")));
			return null;
		}
		SuperInvocation builder = new SuperInvocation(this, classBuilder.getForDefinition(), delegation, invocation,
		...
		try {
			...
			if ((Strategy.generateInstantiator(builder.getPrimaryDeclaration())
					|| builder.getPrimaryDeclaration() instanceof Class) && outerDeclaration instanceof Interface
				...
				if (expr == null) {
					...
					if (ModelUtil.getClassOrInterfaceContainer(classBuilder.getForDefinition(),
							false) instanceof Interface) {
			...
			final List<JCExpression> superArguments = transformSuperInvocationArguments(classBuilder, builder,
					callBuilder);
	...
	/**
	 * Transforms the arguments for the invocation of a superclass initializer (call
	 * to {@code super()}).
	 * 
	 * This is complicated by the need to avoid #929, so when a backward branch is
	 * needed in the evaluation of any argument expression we generate methods on
	 * the companion class (one for each argument) to evaluate the arguments so that
	 * the uninitialized {@code this} is not on the operand stack.
	 */
	private List<JCExpression> transformSuperInvocationArguments(ClassDefinitionBuilder classBuilder,
			SuperInvocation invocation, CallBuilder callBuilder) {
	...
	public JCExpression transform(Tree.InvocationExpression ce) {
		...
		if (ce.getPositionalArgumentList() != null) {
			if ((isIndirectInvocation(ce) || isWithinDefaultParameterExpression(primaryDeclaration.getContainer()))
					&& !Decl.isJavaStaticOrInterfacePrimary(ce.getPrimary())) {
				// indirect invocation
				invocation = new IndirectInvocation(this, primary, primaryDeclaration, ce);
			} else {
				// direct invocation
				java.util.List<Parameter> parameters = ((Functional) primaryDeclaration).getFirstParameterList()
						.getParameters();
				invocation = new PositionalInvocation(this, primary, primaryDeclaration, producedReference, ce,
						parameters);
			}
	...
	public JCExpression transformFunctional(Tree.StaticMemberOrTypeExpression expr, Functional functional,
			Type expectedType) {
		return CallableBuilder.methodReference(gen(), expr, functional.getFirstParameterList(), expectedType,
				expr.getTypeModel(), true);
	}

	public JCExpression transformFunctionalInterfaceBridge(Tree.StaticMemberOrTypeExpression expr, Value functional,
			Type expectedType) {
	...
	public JCExpression transformFunctionalInterfaceBridge(Tree.InvocationExpression expr, JCExpression primaryExpr,
			Type expectedType) {
		ParameterList paramList = new ParameterList();
		int i = 0;
		Type callableType = expr.getTypeModel().getSupertype(typeFact().getCallableDeclaration());
		...
		return CallableBuilder.callableToFunctionalInterface(gen(), expr, paramList, expectedType, expr.getTypeModel(),
				false, primaryExpr);
	}

	public JCExpression transformCallableBridge(Tree.StaticMemberOrTypeExpression expr, Value functional,
			Type expectedType) {
		ParameterList paramList = new ParameterList();
		// expr is a SAM
	...
	JCExpression transformMemberReference(Tree.QualifiedMemberOrTypeExpression expr,
		...
		try {
			if (member.isStatic()) {
				if (member instanceof Function) {
					...
					Reference producedReference = expr.getTarget();// method.appliedReference(qualifyingType,
																	// typeArguments.getTypeModels());
					return CallableBuilder.javaStaticMethodReference(gen(), expr, expr.getTypeModel(), method,
							producedReference, expectedTypeIfCoerced).build();
					...
					CallBuilder callBuilder = CallBuilder.instance(this);
					JCExpression qualExpr = naming.makeTypeDeclarationExpression(null,
							(TypeDeclaration) member.getContainer(), DeclNameFlag.QUALIFIED);
					...
					callBuilder.invoke(naming.makeQualifiedName(qualExpr, (TypedDeclaration) member,
							Naming.NA_GETTER | Naming.NA_MEMBER));

					return callBuilder.build();
				} else if (member instanceof Class) {
					Reference producedReference = expr.getTarget();
					return CallableBuilder.javaStaticMethodReference(gen(), expr, expr.getTypeModel(), (Class) member,
							producedReference, expectedTypeIfCoerced).build();
				}
			}
			if (member instanceof Value) {
				if (expr.getStaticMethodReference() && ModelUtil.isEnumeratedConstructor((Value) member)) {
					...
					if (class1.isToplevel() || class1.isStatic()) {
						qualExpr = naming.makeTypeDeclarationExpression(null,
								class1.isStatic() ? (TypeDeclaration) class1.getContainer() : class1,
								DeclNameFlag.QUALIFIED);
						callBuilder.invoke(naming.makeQualifiedName(qualExpr, (TypedDeclaration) member,
								Naming.NA_GETTER | Naming.NA_MEMBER));
					} else if (class1.isMember()) {
						// Stef: this is fugly but I couldn't find better. This makes sure that
						// Outer.Inner.enumeratedConstructor
						// creates a Callable<Outer.Inner,[Outer]> that returns the
						// enumeratedConstructor given an outer instance
						if (primary instanceof Tree.QualifiedMemberOrTypeExpression
							...
							return CallableBuilder.unboundValueMemberReference(gen(), expr, expr.getTypeModel(),
									((TypedDeclaration) member), expectedTypeIfCoerced).build();
						else {
							qualExpr = primary instanceof Tree.QualifiedMemberOrTypeExpression
									? transformExpression(((Tree.QualifiedMemberOrTypeExpression) primary).getPrimary())
									: null;
							callBuilder.invoke(naming.makeQualifiedName(qualExpr, (TypedDeclaration) member,
									Naming.NA_GETTER | Naming.NA_MEMBER));
						}
						...
						qualExpr = naming.makeQualifiedName(null, (TypedDeclaration) member,
								Naming.NA_Q_LOCAL_INSTANCE);
				...
				Reference producedReference = expr.getTarget();
				return CallableBuilder.unboundFunctionalMemberReference(gen(), expr, expr.getTypeModel(),
						ModelUtil.getConstructor(member), producedReference, expectedTypeIfCoerced);
			} else if (member instanceof Function) {
				Function method = (Function) member;
				if (!method.isParameter()) {
					Reference producedReference = method.appliedReference(qualifyingType,
							typeArguments.getTypeModels());
					return CallableBuilder.unboundFunctionalMemberReference(gen(), expr, expr.getTypeModel(), method,
							producedReference, expectedTypeIfCoerced);
				} else {
					Reference producedReference = method.appliedReference(qualifyingType,
							typeArguments.getTypeModels());
					return CallableBuilder.unboundFunctionalMemberReference(gen(), expr, expr.getTypeModel(), method,
				...
				Reference producedReference = expr.getTarget();
				return CallableBuilder.unboundFunctionalMemberReference(gen(), expr, expr.getTypeModel(),
						(Class) member, producedReference, expectedTypeIfCoerced);
			} else {
	...
	private JCExpression transformSafeMemberOperator(Tree.QualifiedMemberOrTypeExpression expr,
	...
	private JCExpression transformSpreadOperator(final Tree.QualifiedMemberOrTypeExpression expr,
			TermTransformer transformer) {
		at(expr);

		boolean spreadMethodReferenceOuter = !expr.equals(spreading) && !isWithinInvocation()
				&& isCeylonCallableSubtype(expr.getTypeModel());
		...
		try {
			...
			if (typeFact().isIterableType(expr.getPrimary().getTypeModel())) {
				...
				srcIterableType = typeFact().getJavaIterableDeclaration().appliedType(null,
						Collections.singletonList(srcElementType));
			...
			if (spreadMethodReferenceInner) {
				...
				if (isSuperOrSuperOf) {
					// in this case we can't capture the iterable because it may be a mixin impl
					// class, but it's constant
					// so we just refer to it later
					if (isSuper) {
						Declaration member = expr.getPrimary().getTypeModel().getDeclaration().getMember("iterator",
								null, false);
			...
			/*
			 * public Object next() { Object result; if (!((result = iterator.next())
			 * instanceof Finished)) { result = transformedMember(result); } return result;
			 */

			/*
			 * Any arguments in the member of the spread would get re-evaluated on each
			 * iteration so we need to shift them to the scope of the Let to ensure they're
			 * evaluated once.
			 */
			boolean aliasArguments = transformer instanceof InvocationTermTransformer
					&& ((InvocationTermTransformer) transformer).invocation
							.getNode() instanceof Tree.InvocationExpression
					&& ((Tree.InvocationExpression) ((InvocationTermTransformer) transformer).invocation.getNode())
							.getPositionalArgumentList() != null;
			if (aliasArguments) {
				((InvocationTermTransformer) transformer).callBuilder.argumentHandling(CallBuilder.CB_ALIAS_ARGS,
						varBaseName);
			}
			...
			try {
				...
				JCExpression transformedElement = applyErasureAndBoxing(iteratorResultName.makeIdent(),
						typeFact().getAnythingType(), CodegenUtil.hasTypeErased(expr.getPrimary()), true,
						BoxingStrategy.BOXED, srcElementType, 0);
				...
				// The code has been called recursively and the part after this if-statement
				// will
				...
				transformedElement = applyErasureAndBoxing(transformedElement, resultElementType,
						// don't trust the erased flag of expr, as it reflects the result type of the
						// overall spread expr,
				...
				if (typeFact().isIterableType(expr.getPrimary().getTypeModel())) {
					...
					JCVariableDecl srcIterator = makeVar(Flags.FINAL, srcIteratorName,
							makeJavaType(typeFact().getIteratorType(srcElementType)), make().Apply(null,
									// for super we do not capture it because we can't and it's constant anyways
									naming.makeQualIdent(
											isSuperOrSuperOf ? srcIterableExpr : srcIterableName.makeIdent(),
											"iterator"),
									List.<JCExpression>nil()));

					resultAbsentType = typeFact().getIteratedAbsentType(expr.getPrimary().getTypeModel());
					nextMdb.body(List.of(makeVar(iteratorResultName, make().Type(syms().objectType), null),
							make().If(
									make().Unary(JCTree.Tag.NOT,
											make().TypeTest(
													make().Assign(iteratorResultName.makeIdent(),
															make().Apply(null,
																	naming.makeQualIdent(srcIteratorName.makeIdent(),
																			"next"),
																	List.<JCExpression>nil())),
													make().Type(syms().ceylonFinishedType))),
									make().Block(0,
											List.<JCStatement>of(make().Exec(make()
													.Assign(iteratorResultName.makeIdent(), transformedElement)))),
									null),
							make().Return(iteratorResultName.makeIdent())));
					l = List.of(srcIterator, nextMdb.build());
				} else if (typeFact().isJavaIterableType(expr.getPrimary().getTypeModel())) {
					// private Iterator<srcElementType> iterator = srcIterableName.iterator();
					JCVariableDecl srcIterator = makeVar(Flags.PRIVATE | Flags.FINAL, srcIteratorName,
							makeJavaType(typeFact().getJavaIteratorType(srcElementType)), make().Apply(null,
									// for super we do not capture it because we can't and it's constant anyways
									naming.makeQualIdent(
											isSuperOrSuperOf ? srcIterableExpr : srcIterableName.makeIdent(),
											"iterator"),
					...
					nextMdb.body(List.<JCStatement>of(make().If(
							make().Apply(null, naming.makeQualIdent(srcIteratorName.makeIdent(), "hasNext"), List
									.<JCExpression>nil()),
							make().Block(
									0, List
											.<JCStatement>of(
													makeVar(iteratorResultName, make().Type(syms().objectType),
															make().Apply(null,
																	naming.makeQualIdent(srcIteratorName.makeIdent(),
																			"next"),
																	List.<JCExpression>nil())),
					...
					JCVariableDecl srcIndex = makeVar(Flags.PRIVATE, srcIteratorName, make().Type(syms().intType),
							make().Literal(0));
					JCExpression indexed = make().Indexed(srcIterableName.makeIdent(),
							make().Unary(Tag.POSTINC, srcIteratorName.makeIdent()));
					if (typeFact().isJavaPrimitiveArrayType(expr.getPrimary().getTypeModel())) {
						indexed = applyErasureAndBoxing(indexed, srcElementType, false, BoxingStrategy.BOXED,
								srcElementType);
					}
					nextMdb.body(List.<JCStatement>of(make().If(make().Binary(Tag.LT, srcIteratorName.makeIdent(),
							// for super we do not capture it because we can't and it's constant anyways
							naming.makeQualIdent(isSuperOrSuperOf ? srcIterableExpr : srcIterableName.makeIdent(),
									"length")),
							make().Block(0,
									List.<JCStatement>of(
											makeVar(iteratorResultName, make().Type(syms().objectType), indexed),
											make().Return(transformedElement))),
							make().Return(makeFinished()))));
				...
				JCNewClass iteratorClass = make().NewClass(null, null,
				...
				iteratorMdb.resultType(new TransformedType(makeJavaType(typeFact().getIteratorType(resultElementType)),
						makeAtNonNull()));
				iteratorMdb.body(make().Return(iteratorClass));

				// new AbstractIterable()
				iterableClass = make().NewClass(null, null,
						make().TypeApply(make().QualIdent(syms().ceylonAbstractIterableType.tsym),
								List.of(makeJavaType(resultElementType, JT_TYPE_ARGUMENT),
										makeJavaType(resultAbsentType, JT_TYPE_ARGUMENT))),
			...
			JCMethodInvocation result = make().Apply(null, naming.makeQualIdent(iterableClass, "sequence"),
					List.<JCExpression>nil());
			...
			if (primaryIsSequence) {
				...
				spread = applyErasureAndBoxing(spread, typeFact().getSequentialType(returnElementType), false, true,
						BoxingStrategy.BOXED, primaryIsSequence ? typeFact().getSequenceType(returnElementType)
	...
	JCExpression transformQualifiedMemberPrimary(Tree.QualifiedMemberOrTypeExpression expr) {
		...
		// do not consider the primary to be an invocation since in foo.x() we're
		// invoking x, not foo.
		boolean previousWithinInvocation = withinInvocation(false);
		try {
			// consider package qualifiers as non-prefixed, we always qualify them anyways,
			// this is
			...
			if (exprDec.isConstructor()) {
				...
				if (primary instanceof Tree.BaseMemberExpression) {
					...
					// constructors of member classes, and ref to constructors of
					// static classes
					primary = qte.getStaticMethodReference() && qte.getDeclaration().isStatic() ? null
							: qte.getPrimary();
				}
			}
			...
			BoxingStrategy boxing = !safeMemberOp && Decl.isValueTypeDecl(primary) && CodegenUtil.isUnBoxed(primary)
			...
			if (isSuper(primary)) {
			...
			} else if (isThis(primary) && !exprDec.isCaptured() && !exprDec.isShared()
				...
				result = transformJavaStaticOrInterfaceMember((Tree.QualifiedMemberOrTypeExpression) primary,
						expr.getTypeModel());
			} else {
	...
	private JCExpression transformJavaStaticOrInterfaceMember(Tree.QualifiedMemberOrTypeExpression qmte,
			Type staticType) {
		...
		if (decl instanceof FieldValue) {
			...
			callBuilder.invoke(naming.makeQualifiedName(makeJavaType(qualifyingType, JT_RAW | JT_NO_PRIMITIVES), member,
					Naming.NA_GETTER | Naming.NA_MEMBER));
			...
			return utilInvocation()
					.checkNull(makeJavaStaticInvocation(gen(), method, producedReference, parameterList));
			...
			if (class_.isStatic()) {
				...
				return utilInvocation()
						.checkNull(makeJavaStaticInvocation(gen(), class_, producedReference, parameterList));
			}
	...
	JCExpression makeJavaStaticInvocation(CeylonTransformer gen, final Functional methodOrClass,
			Reference producedReference, final ParameterList parameterList) {
		CallBuilder callBuilder = CallBuilder.instance(gen);
		if (methodOrClass instanceof Function) {
			JCExpression fn;
			Declaration dec = (Declaration) methodOrClass;
			fn = Decl.isJavaArrayFrom(dec) ? gen.makeUnwrapArray(dec)
					: naming.makeName((Function) methodOrClass, Naming.NA_FQ | Naming.NA_WRAPPER_UNQUOTED);
			callBuilder.invoke(fn);
		} else if (methodOrClass instanceof Class) {
			Class klass = (Class) methodOrClass;
			callBuilder.instantiate(gen.makeJavaType(klass.getType(), JT_RAW | JT_NO_PRIMITIVES));
		}
	...
	/**
	 * Is the given primary a {@code super of Foo} expression (modulo parentheses
	 * and multiple {@code of}
	 */
	...
	/**
	 * Is the given primary a {@code super} expression (modulo parentheses)
	 */
	...
	/**
	 * Is the given primary a {@code super} or {@code super of Foo} expression
	 * (modulo parentheses and multiple {@code of}
	 */
	...
	private JCExpression widenSuper(Node superOfQualifiedExpr, TypeDeclaration inheritedFrom) {
		JCExpression result;
		if (inheritedFrom instanceof Class) {
			...
			result = makeErroneous(superOfQualifiedExpr,
					"compiler bug: " + (inheritedFrom == null ? "null" : inheritedFrom.getClass().getName())
							+ " is an unhandled case in widen()");
		}
		return result;
	}
	...
	public JCExpression transformTermForInvocation(Tree.Term term, TermTransformer transformer) {
		if (term instanceof Tree.QualifiedMemberExpression) {
			...
			// do not consider our term to be part of an invocation, we want it to be a
			// Callable
	...
	private JCExpression transformMemberExpression(Tree.StaticMemberOrTypeExpression expr, JCExpression primaryExpr,
		...
		// Try to find the original declaration, in case we have conditionals that
		// refine the type of objects without us
		...
		// Explanation: primaryExpr and qualExpr both specify what is to come before the
		// selector
		// but the important difference is that primaryExpr is used for those situations
		// where
		// the result comes from the actual Ceylon code while qualExpr is used for those
		// situations
		// where we need to refer to synthetic objects (like wrapper classes for
		// toplevel methods)

		JCExpression qualExpr = null;
		String selector = null;
		// true for Java interop using fields, and for super constructor parameters,
		// which must use
		...
		if (decl instanceof Functional && (!(decl instanceof Class) || ((Class) decl).getParameterList() != null)
		...
		} else if (coerced && decl instanceof Value && isFunctionalResult(expr.getTypeModel())
				&& checkForFunctionalInterface(expectedType) != null) {
			result = transformFunctionalInterfaceBridge(expr, (Value) decl, expectedType);
		} else if (coerced && decl instanceof Functional && decl.isParameter()
				&& isFunctionalResult(expr.getTypeModel()) && checkForFunctionalInterface(expectedType) != null) {
			result = transformFunctional(expr, (Functional) decl, expectedType);
			// } else if (coerced
			...
			if (decl.isToplevel()) {
				...
				mustUseField = Decl.isJavaField(decl)
						|| (isWithinSuperInvocation() && primaryExpr == null && withinSuperInvocation == container);
				...
				if (mustUseField || mustUseParameter) {
					...
					// invoke the getter, using the Java interop form of Util.getGetterName because
				...
				if (ModelUtil.isLocalNotInitializer(typeDecl) && typeDecl.isAnonymous()
				...
				} else if (decl.isCaptured() && !typedDecl.isVariable()
			...
			mustUseParameter = primaryExpr == null && decl.isParameter()
					&& isWithinDefaultParameterExpression(decl.getContainer());
			if (!decl.isParameter() && (ModelUtil.isLocalNotInitializer(decl)
					|| ModelUtil.isLocalToInitializer(decl) && ((Function) decl).isDeferred())) {
				primaryExpr = null;
		...
		if (result == null) {
			...
			if (decl instanceof Value && ModelUtil.isEnumeratedConstructor((Value) decl)) {
				...
				if (constructedClass.isToplevel() || constructedClass.isClassMember())
					...
					qualExpr = naming.makeQualifiedName(primaryExpr, (TypedDeclaration) decl,
							Naming.NA_Q_LOCAL_INSTANCE);
					selector = naming.selector((TypedDeclaration) decl);
				}
			}
			if (qualExpr == null && (decl.isStatic() || isEnumeratedConstructorGetter)
			...
			// and not classes
				...
				qualExpr = naming.makeTypeDeclarationExpression(null, (TypeDeclaration) decl.getContainer(),
						DeclNameFlag.QUALIFIED);
			}
			...
			if (transformer != null) {
				...
				if ((Decl.isValueTypeDecl(qmePrimary) || safeMemberOp && javaArray)
					...
					JCExpression primTypeExpr = makeJavaType(qmePrimary.getTypeModel(),
							JT_NO_PRIMITIVES | JT_VALUE_TYPE);
					result = makeQualIdent(primTypeExpr, selector);
					result = make().Apply(List.<JCTree.JCExpression>nil(), result,
		...
		if (transformer == null && decl instanceof TypedDeclaration
			...
			result = make().Apply(null, makeQualIdent(result, Naming.Unfix.apply.toString()),
					List.<JCExpression>of(make().NewArray(make().Type(syms().ceylonTypeDescriptorType),
							List.<JCExpression>nil(), tds.toList())));
		}
	...
	/**
	 * We may need to force a qualified this prefix (direct or outer) in the
	 * following cases:
	 * 
	 * - Required because of mixin inheritance with different type arguments (the
	 * same is already done for qualified references, but not for direct references)
	 * - The compiler generates anonymous local classes for things like Callables
	 * and Comprehensions. When referring to a member foo within one of those things
	 * we need a qualified {@code this} to ensure we're accessing the outer
	 * instances member, not a member of the anonymous local class that happens to
	 * have the same name.
	 */
	private JCExpression addThisOrObjectQualifierIfRequired(JCExpression qualExpr,
		...
		if (qualExpr == null
				...
				&& !decl.isStatic() && (constructor ? !typeDecl.isStatic() : true) && typeDecl.isMember()
				// dodge variable refinements with assert/is (these will be turned to locals
				// and have a name mapping)
				&& target.getDeclaration() == decl && !ModelUtil.isLocalToInitializer(typeDecl)
			...
			if (outer != null) {
				...
				ClassOrInterface classOrInterface = (ClassOrInterface) typeDecl.getContainer();
				if (classOrInterface.isAnonymous() && classOrInterface.isToplevel()) {
					// easy
	...
	/**
	 * §3.2.2 Every interface is a subtype of c.l.Object, so within an Interface
	 * {@code string} means {@code $this.toString()}
	 * 
	 * @param expr
	 * @param decl
	 * @param qualExpr
	...
	private JCExpression addQualifierForObjectMembersOfInterface(Tree.StaticMemberOrTypeExpression expr,
			Declaration decl, JCExpression qualExpr) {
		if (expr instanceof Tree.BaseMemberExpression && qualExpr == null
	...
	/**
	 * Determines whether we need to generate an AbstractCallable when taking a
	 * method reference to a method that's declared as a FunctionalParameter
	 */
	private boolean functionalParameterRequiresCallable(Function functionalParameter,
		...
		if (!hasMethod) {
			// A functional parameter that's not method wrapped will already be
			// Callable-wrapped
			return false;
		}
	...
	private JCExpression addInterfaceImplAccessorIfRequired(JCExpression qualExpr,
			Tree.StaticMemberOrTypeExpression expr, Declaration decl) {
		// Partial fix for https://github.com/ceylon/ceylon-compiler/issues/1023
		// For interfaces we sometimes need to access either the interface instance or
		// its $impl class
		...
		if (qualExpr != null
				...
				&& !decl.isShared() && (!(expr instanceof Tree.QualifiedMemberExpression)
	...
	private JCExpression makeQualifiedDollarThis(Tree.BaseMemberExpression expr) {
		...
		while (scope != null) {
			if (scope instanceof Interface) {
				...
				// we only need to qualify it if we're aiming for a $this of an outer interface
				// than the interface we are caught in
				needsQualified = true;
			}
			scope = scope.getContainer();
		}
	...
	/**
	 * Abstract method pattern for transforming indexed expressions:
	 * {@code foo[index]}
	 */
	abstract class AbstractIndexTransformer {
		...
		public JCExpression transform(Tree.IndexExpression indexExpr) {
			...
			if (!expectedType.isExactly(elementType)
				...
				flags |= EXPR_DOWN_CAST;
			result = applyErasureAndBoxing(result, elementType, CodegenUtil.hasTypeErased(indexExpr), true,
					BoxingStrategy.BOXED, expectedType, flags);
			return result;
		}
		...
		protected JCExpression transformIndexed(Tree.IndexExpression indexExpr) {
			...
			JCExpression result = at(indexExpr).Apply(List.<JCTree.JCExpression>nil(), primaryExpr, List.of(index));
			return result;
		}
	...
	class CorrespondenceIndexTransformer extends AbstractIndexTransformer {
		...
		CorrespondenceIndexTransformer(Tree.IndexExpression indexExpr, Type leftType, Type rightType,
				Type sequentialElementType) {
			super(indexExpr, leftType, rightType, sequentialElementType);
			boolean isOnList = primaryType.isSubtypeOf(typeFact().getListDeclaration().appliedType(null,
					Collections.singletonList(typeFact().getAnythingType())));
			...
			if (isOnList) {
				...
				if (isOnSuper) {
					// this is super special: if we use the optim and call super we need to make
					// sure that "getFromFirst" already
	...
	class JavaListIndexTransformer extends AbstractIndexTransformer {
		...
		JavaListIndexTransformer(Tree.IndexExpression indexExpr, Type leftType, Type rightType,
				Type sequentialElementType) {
			super(indexExpr, leftType, rightType, sequentialElementType);
		}
		...
		protected JCExpression transformIndexed(Tree.IndexExpression indexExpr) {
			...
			return at(
					indexExpr)
							.LetExpr(
									List.<JCStatement>of(
											makeVar(listName, makeJavaType(leftType), listExpr), makeVar(indexName,
													make().Type(syms().intType), index)),
									make().Conditional(
											make().Binary(JCTree.Tag.AND,
													make().Binary(JCTree.Tag.GE, indexName.makeIdent(),
															make().Literal(0)),
		...
		protected JCExpression transformIndexed(Tree.IndexExpression indexExpr) {
			...
			if (typeFact().isOptionalType(returnType)) {
				...
				return at(indexExpr).LetExpr(
						List.<JCStatement>of(makeVar(listName, makeJavaType(leftType, JT_NO_PRIMITIVES), arrayExpr),
						...
						make().Conditional(
								...
								make().Indexed(listName.makeIdent(), indexName.makeIdent()), makeNull()));
			} else {
	...
	public JCExpression transform(Tree.IndexExpression indexedExpr) {
		...
		if (elementOrRange instanceof Tree.Element) {
			...
			if (leftType != null) {
				...
				transformer = new CorrespondenceIndexTransformer(indexedExpr, leftType, rightType,
						getTypeArgument(leftType, 1));
			} else {
				...
				if (leftType != null) {
					...
					transformer = new JavaListIndexTransformer(indexedExpr, leftType, rightType,
							getTypeArgument(leftType, 0));
				} else {
					leftType = primaryType.getSupertype(typeFact().getJavaMapDeclaration());
					if (leftType != null) {
						Type rightType = getTypeArgument(leftType, 0);
						transformer = new JavaMapIndexTransformer(indexedExpr, leftType, rightType,
								getTypeArgument(leftType, 1));
			...
			if (range.getLowerBound() != null && range.getLength() != null) {
				method = "measure";
				JCExpression length = transformExpression(range.getLength(), BoxingStrategy.UNBOXED,
						typeFact().getIntegerType());
			...
			if (isSuper || isSuperOf(primary)) {
				...
				int flags = 0;
				// this is pretty much disgusting, but all I found. Given a
				// Ranged<Integer,Integer,Integer[]> type
				// we don't notice that this type can't be cast to without a raw cast because it
				// has constrained
				// type params in its hierarchy. hasConstrainedTypeParameters only checks that
				// type, and we want
				// to check Ranged<Index, Element, Subrange> instead, which has those
				// constraints, which is why we
				// do it on the declaration type. Honestly this should probably rather be an
				// inheritance check in
				// applyErasureAndCasts but that'd be costly and complex and likely introduce
			...
			// (not a sequential of the union of types in the ranged) a typecast may be
			// required.
			...
			if (!expectedType.isExactly(rangedSpanType)) {
				...
				if (method.equals("spanFrom")) {
					...
					result = makeErroneous(indexedExpr,
							"compiler bug: only the spanFrom method should be specialised for Tuples");
				}
			} else {
				// make a "lhs.<method>(start, end)" call
				result = at(indexedExpr).Apply(List.<JCTree.JCExpression>nil(), makeSelect(lhs, method), args);
			}
			result = applyErasureAndBoxing(result, rangedSpanType, CodegenUtil.hasTypeErased(indexedExpr), true,
					BoxingStrategy.BOXED, expectedType, flags);
			return result;
		}
	}
	...
	private JCExpression transformAssignment(Node op, Tree.Term leftTerm, Tree.Term rightTerm) {
		...
		// FIXME: can this be anything else than a Tree.MemberOrTypeExpression or
		// Tree.ParameterizedExpression or Tree.IndexExpression?
		...
		if (leftTerm instanceof Tree.MemberOrTypeExpression) {
			...
			// if we're dealing with widening do not trust the type of the declaration and
			// get the real type
			...
			if (decl.isMember() && useFieldInAssignment(op, null, decl) && !ModelUtil.isLocalToInitializer(decl)
			...
			FunctionOrValue decl = (FunctionOrValue) ((Tree.MemberOrTypeExpression) paramExpr.getPrimary())
					.getDeclaration();
			CallableBuilder callableBuilder = CallableBuilder.anonymous(gen(), paramExpr, decl,
					(Tree.Expression) rightTerm, paramExpr.getParameterLists(), paramExpr.getPrimary().getTypeModel(),
					decl instanceof Function ? !((Function) decl).isDeferred() : false);
			rhs = callableBuilder.build();
		} else {
			return makeErroneous(leftTerm, "compiler bug: left term of type '" + leftTerm.getClass().getSimpleName()
	...
	private JCExpression transformAssignmentLhs(final Node op, Tree.Term leftTerm) {
		// left hand side can be either BaseMemberExpression, QualifiedMemberExpression
		// or array access (M2)
		...
		if (leftTerm instanceof Tree.BaseMemberExpression) {
			...
			if (isPackageQualified(qualified)) {
				...
				} else if (isThis(primary) && !qualifiedDec.isCaptured() && !qualifiedDec.isShared()) {
					lhs = null;
	...
	protected JCExpression qualifyLhs(final Node op, Tree.Term leftTerm, JCExpression lhs) {
		...
		if (leftTerm instanceof Tree.StaticMemberOrTypeExpression) {
			...
			decl = (TypedDeclaration) ((Tree.MemberOrTypeExpression) ((Tree.ParameterizedExpression) leftTerm)
					.getPrimary()).getDeclaration();
		} else {
		...
		if (decl.isToplevel()) {
			...
			if (Decl.isTransient(decl) && !decl.isVariable()) {
				...
				if (Decl.isLocal(decl)) {
					...
					lhs = naming.makeTypeDeclarationExpression(null, (TypeDeclaration) decl.getContainer(),
							DeclNameFlag.QUALIFIED);
				}
			}
	...
	private JCExpression makeAssignment(Node op, Tree.Term leftTerm, final JCExpression lhs, JCExpression rhs) {
		...
		if (varExpr != null) {
			...
			result = make().Apply(typeArguments.toList(), makeQualIdent(lhs, selector),
	...
	protected JCExpression makeAssignmentVariable(Node op, final JCExpression lhs, TypedDeclaration decl) {
		...
		if (decl.isToplevel()) {
			...
			if (Decl.isBoxedVariable(decl)) {
				...
				varExpr = naming
						.makeUnquotedIdent(Naming.getAliasedParameterName(((Value) decl).getInitializerParameter()));
			}
		} else {
			varExpr = naming.makeQualifiedName(lhs, decl, Naming.NA_IDENT);
		}
	...
	private JCExpression transformIndexAssignment(Node op, Tree.IndexExpression idx, JCExpression rhs) {
		...
		if (idx.getElementOrRange() instanceof Tree.Element) {
			...
			if ((primaryType = pt.getSupertype(unit.getIndexedCorrespondenceMutatorDeclaration())) != null) {
				JCExpression iex = transformExpression(elem.getExpression(), BoxingStrategy.UNBOXED,
						elem.getExpression().getTypeModel());
				...
				JCExpression iex = transformExpression(elem.getExpression(), BoxingStrategy.BOXED,
						elem.getExpression().getTypeModel());
	...
	private JCExpression makeIndexAssignMethod(Tree.IndexExpression leftTerm, String method, JCExpression index,
			JCExpression rhs, Type expectedPrimaryType) {
		JCExpression lhs = transformExpression(leftTerm.getPrimary(), BoxingStrategy.BOXED, expectedPrimaryType);
		return make().Apply(List.<JCTree.JCExpression>nil(), makeQualIdent(lhs, method),
				List.<JCTree.JCExpression>of(index, rhs));
	}

	/**
	 * Creates an anonymous class that extends Iterable and implements the specified
	 * comprehension.
	 */
	...
	private Type wrapInOptionalForInterop(Type elementType, Type expectedType, boolean containsUncheckedNull) {
		if (expectedType != null && containsUncheckedNull && iteratesOverOptional(expectedType)
				&& !typeFact().isOptionalType(elementType))
	...
	enum IterType {
		CEYLON_ITERABLE {
			...
			ListBuffer<JCStatement> makeContext(ExpressionTransformer gen, ComprehensionTransformation ct,
					Tree.ForIterator forIterator, SyntheticName iterVar, SyntheticName itemVar, SyntheticName tmpItem,
				...
				contextBody.add(gen.make().VarDef(gen.make().Modifiers(Flags.FINAL), tmpItem.asName(),
						...
						gen.make().Apply(null, gen.makeSelect(iterVar.makeIdent(), "next"), List.<JCExpression>nil())));
				...
				if (ct.idx > 0) {
					...
					// This will reset our previous context by getting a new iterator if the
					// previous loop isn't done
				...
				contextBody.add(gen.make().If(itemVar.suffixedBy(Suffix.$exhausted$).makeIdent(),
						gen.make().Block(0, innerBody.toList()), gen.make().Block(0, elseBody.toList())));
				return contextBody;
			}
		},
		JAVA_ITERABLE {
			...
			JCExpression makeIteratorType(ExpressionTransformer gen, Type iterType) {
				return gen
						.makeJavaType(gen.typeFact().getJavaIteratorType(gen.typeFact().getJavaIteratedType(iterType)));
			}
			...
			ListBuffer<JCStatement> makeContext(ExpressionTransformer gen, ComprehensionTransformation ct,
					...
					ListBuffer<JCStatement> elseBody) {
				ListBuffer<JCStatement> contextBody = new ListBuffer<JCStatement>();
				contextBody.add(gen.make()
						.Exec(gen.make().Assign(itemVar.suffixedBy(Suffix.$exhausted$).makeIdent(),
								gen.make().Unary(JCTree.Tag.NOT, gen.make().Apply(null,
										gen.makeSelect(iterVar.makeIdent(), "hasNext"), List.<JCExpression>nil())))));
				...
				JCExpression nextInvocation = gen.make().Apply(null, gen.makeSelect(iterVar.makeIdent(), "next"),
						List.<JCExpression>nil());
				...
				contextBody
						.add(gen.make()
								.If(itemVar.suffixedBy(Suffix.$exhausted$).makeIdent(),
										gen.make().Block(0, innerBody.toList()),
										gen.make()
												.Block(0,
														List.<JCStatement>of(
																gen.make()
																		.Exec(gen.make().Assign(itemVar.makeIdent(),
																				nextInvocation)),
		...
		JAVA_ARRAY {
			@Override
			void makeInitial(ExpressionTransformer gen, ComprehensionTransformation ct, SyntheticName iterVar,
				...
				ct.fields.add(gen.make().VarDef(gen.make().Modifiers(Flags.PRIVATE), iterVar.asName(),
						gen.make().Type(gen.syms().intType), null));
				ct.fields.add(gen.make().VarDef(gen.make().Modifiers(Flags.PRIVATE | Flags.FINAL), arrayVar.asName(),
						iterTypeExpr, iterableExpr));
			...
			@Override
			ListBuffer<JCStatement> makeSubsequent(ExpressionTransformer gen, ComprehensionTransformation ct,
					Type iterType, SyntheticName iterVar, JCExpression iterableExpr) {
				SyntheticName arrayVar = arrayVar(gen, ct);
				ListBuffer<JCStatement> block = new ListBuffer<JCStatement>();
				ct.fields.add(gen.make().VarDef(gen.make().Modifiers(Flags.PRIVATE), iterVar.asName(),
						gen.make().Type(gen.syms().intType), null));
				ct.fields.add(gen.make().VarDef(gen.make().Modifiers(Flags.PRIVATE), arrayVar.asName(),
						makeIteratorType(gen, iterType), null));

				block.appendList(List.<JCStatement>of(
						gen.make().If(gen.make().Binary(JCTree.Tag.NE, arrayVar.makeIdent(), gen.makeNull()),
								gen.make().Return(gen.makeBoolean(true)), null),
						gen.make().If(
								gen.make().Unary(JCTree.Tag.NOT,
										gen.make().Apply(null, ct.ctxtName.makeIdentWithThis(),
												List.<JCExpression>nil())),
								gen.make().Return(gen.makeBoolean(false)), null)));
				block.appendList(ct.capture());
				block.appendList(
						List.<JCStatement>of(gen.make().Exec(gen.make().Assign(arrayVar.makeIdent(), iterableExpr)),
			...
			ListBuffer<JCStatement> makeContext(ExpressionTransformer gen, ComprehensionTransformation ct,
				...
				contextBody.add(gen.make()
						.Exec(gen.make().Assign(itemVar.suffixedBy(Suffix.$exhausted$).makeIdent(),
								gen.make().Binary(JCTree.Tag.GE, iterVar.makeIdent(),
										gen.naming.makeSelect(arrayVar.makeIdent(), "length")))));
				...
				indexed = gen.applyErasureAndBoxing(indexed, gen.typeFact().getJavaArrayElementType(iterType),
						gen.typeFact().getJavaObjectArrayDeclaration()
								.equals(iterType.resolveAliases().getDeclaration()),
						CodegenUtil.getBoxingStrategy(varModel), varModel.getType());
		...
		void makeInitial(ExpressionTransformer gen, ComprehensionTransformation ct, SyntheticName iterVar,
				Type iterType, JCExpression iterableExpr) {
			JCExpression iterTypeExpr = makeIteratorType(gen, iterType);
			ct.fields.add(gen.make().VarDef(gen.make().Modifiers(Flags.PRIVATE | Flags.FINAL), iterVar.asName(),
					iterTypeExpr, null));
			ct.initIterator = gen.make().Exec(gen.make().Assign(iterVar.makeIdent(),
					gen.make().Apply(null, gen.makeSelect(iterableExpr, "iterator"), List.<JCExpression>nil())));
		}

		/**
		 * Transform the non-initial iterator, declaring fields within the given
		 * {@code ct}.
		 */
		ListBuffer<JCStatement> makeSubsequent(ExpressionTransformer gen, ComprehensionTransformation ct, Type iterType,
				SyntheticName iterVar, JCExpression iterableExpr) {
			ListBuffer<JCStatement> block = new ListBuffer<JCStatement>();
			ct.fields.add(gen.make().VarDef(gen.make().Modifiers(Flags.PRIVATE), iterVar.asName(),
					makeIteratorType(gen, iterType), null));
			block.appendList(
			...
			block.appendList(
					List.<JCStatement>of(
							gen.make()
									.Exec(gen.make().Assign(iterVar.makeIdent(),
											gen.make().Apply(null, gen.makeSelect(iterableExpr, "iterator"),
													List.<JCExpression>nil()))),
							gen.make().Return(gen.makeBoolean(true))));
			return block;
		}
	...
	class ComprehensionTransformation {
		...
		public JCExpression transformComprehension() {
			at(comp);
			// make sure "this" will be qualified since we're introducing a new surrounding
			// class
			...
			try {
				...
				while (clause != null) {
					...
					if (clause instanceof Tree.ForComprehensionClause) {
						...
						return makeErroneous(clause, "compiler bug: comprehension clauses of type "
								+ clause.getClass().getName() + " are not yet supported");
					}
					idx++;
					if (itemVar != null)
						prevItemVar = itemVar;
		...
		private JCMethodDecl makeNextMethod(Type iteratedType) {
			List<JCStatement> of = List.<JCStatement>of(
					make().Return(transformExpression(excc.getExpression(), BoxingStrategy.BOXED, iteratedType)));
			of = of.prependList(capture());
			JCStatement stmt = make().If(make().Apply(null, ctxtName.makeIdentWithThis(), List.<JCExpression>nil()),
					make().Block(0, of), make().Return(makeFinished()));
			return make().MethodDef(make().Modifiers(Flags.PUBLIC | Flags.FINAL), names().fromString("next"),
					...
					List.<JCTree.JCVariableDecl>nil(), List.<JCExpression>nil(),
					make().Block(0, List.<JCStatement>of(stmt)), null);
		}

		/**
		 * Builds a {@code getIterator()} method which contains a local class extending
		 * {@code AbstractIterator} and initialises the iter$0 field to a new instance
		 * of that local class.
		 * 
		...
		private JCMethodDecl makeGetIterator(Type iteratedType) {
			...
			JCExpression iterator = at(comp).NewClass(null, List.<JCExpression>nil(), iteratorTypeExpr,
					...
					make().AnonymousClassDef(make().Modifiers(0), fields.toList().prepend(make().Block(0L,
							initIterator == null ? List.<JCStatement>nil() : List.<JCStatement>of(initIterator)))));
			JCBlock iteratorBlock = make().Block(0, List.<JCStatement>of(make().Return(iterator)));
			return make().MethodDef(make().Modifiers(Flags.PUBLIC | Flags.FINAL), names().fromString("iterator"),
					makeJavaType(iteratorType, JT_CLASS_NEW | JT_EXTENDS), List.<JCTree.JCTypeParameter>nil(),
					List.<JCTree.JCVariableDecl>nil(), List.<JCExpression>nil(), iteratorBlock, null);
		}

		/**
		 * Builds an anonymous subclass of AbstractIterable whose {@code getIterator()}
		 * uses the given getIteratorBody.
		 * 
		 * @param iteratedType
		 * @param iteratorType
		 ...
		 * @return
		 */
		private JCExpression makeAnonymousIterable(Type iteratedType, JCMethodDecl getIterator) {
			JCExpression iterable = make().NewClass(null, null,
					...
					make().AnonymousClassDef(make().Modifiers(0), List.<JCTree>of(getIterator)));
			return iterable;
		}
		...
		class IfComprehensionCondList extends CondList {
			 ...
			 * Checks {@code condExpr} before checking the {@code conditions}, and
			 * {@code break;}s if the conditions apply. Intended to be placed in a
			 * {@code while (true) } loop, to keep checking the conditions until they apply
			...
			public IfComprehensionCondList(java.util.List<Tree.Condition> conditions, JCExpression condExpr,
				...
				this(conditions,
						...
						capture().prepend(
								make().If(make().Unary(JCTree.Tag.NOT, condExpr), make().Break(breakLabel), null)),
						// break if a condition matches
						List.<JCStatement>of(make().Break(breakLabel)), null);
			}

			/**
			 * General-purpose constructor. Places {@code precheck} before the conditions
			 * and their variable declarations, {@code insideCheck} in the body of the
			 * innermost condition (executed only if all {@code conditions} apply), and
			 * {@code postCheck} after the conditions.
			 */
			public IfComprehensionCondList(java.util.List<Tree.Condition> conditions, List<JCStatement> preCheck,
					List<JCStatement> insideCheck, List<JCStatement> postCheck) {
				statementGen().super(conditions, (Tree.Block) null);
				if (preCheck == null)
					preCheck = List.<JCStatement>nil();
				if (insideCheck == null)
					insideCheck = List.<JCStatement>nil();
				if (postCheck == null)
					postCheck = List.<JCStatement>nil();
			...
			protected List<JCStatement> transformInnermost(Tree.Condition condition) {
				...
				return transformCommon(transformedCond.getVarTrans(), test, insideCheck, vars);
			}

			protected List<JCStatement> transformIntermediate(Tree.Condition condition,
			...
			protected List<JCStatement> transformCommon(VarTrans vartrans, JCExpression test, List<JCStatement> stmts,
				...
				stmts = List.<JCStatement>of(make().If(test, make().Block(0, stmts), null));
				return stmts;
			}
		...
		private void transformIfClause(Tree.IfComprehensionClause clause) {
			...
			if (prevItemVar == null) {
				...
				if (clause == comp.getInitialComprehensionClause()) {
					...
					JCVariableDecl exhaustedDef = make().VarDef(make().Modifiers(Flags.PRIVATE), exhaustedName.asName(),
							makeJavaType(typeFact().getBooleanType()), null);
					fields.add(exhaustedDef);
					JCStatement returnIfExhausted = make().If(exhaustedName.makeIdent(),
							make().Return(makeBoolean(false)), null);
					JCStatement setExhaustedTrue = make()
							.Exec(make().Assign(exhaustedName.makeIdent(), makeBoolean(true)));
					...
					JCStatement returnIfExhausted = make().If(
							...
							make().Unary(JCTree.Tag.NOT,
									make().Apply(null, ctxtName.makeIdentWithThis(), List.<JCExpression>nil())),
							// return false
							make().Return(makeBoolean(false)), null);
					ctxtName = naming.synthetic(Prefix.$next$, idx);
				...
				body = new IfComprehensionCondList(clause.getConditionList().getConditions(), initBlock,
						List.<JCStatement>of(
				...
				// Filter contexts need to check if the previous context applies and then check
				// the condition
				JCExpression condExpr = make().Apply(null, ctxtName.makeIdentWithThis(), List.<JCExpression>nil());
				ctxtName = naming.synthetic(Prefix.$next$, idx);
				Name label = names().fromString("ifcomp_" + idx);
				IfComprehensionCondList ifComprehensionCondList = new IfComprehensionCondList(
						clause.getConditionList().getConditions(), condExpr, label);
				...
				body = List.<JCStatement>of(loop, make()
						.Return(make().Unary(JCTree.Tag.NOT, prevItemVar.suffixedBy(Suffix.$exhausted$).makeIdent())));
			}
			MethodDefinitionBuilder mb = MethodDefinitionBuilder
					.systemMethod(ExpressionTransformer.this, ctxtName.getName()).ignoreModelAnnotations()
					.modifiers(Flags.PRIVATE | Flags.FINAL)
					.resultType(new TransformedType(makeJavaType(typeFact().getBooleanType()))).body(body);
			fields.add(mb.build());
		}
		...
		private SyntheticName transformForClause(final Tree.ForComprehensionClause clause,
			...
			JCExpression iterableExpr = transformExpression(specexpr.getExpression(), BoxingStrategy.BOXED,
					iterableType);
			if (clause == comp.getInitialComprehensionClause()) {
				...
				fields.add(make().MethodDef(make().Modifiers(Flags.PRIVATE | Flags.FINAL), iterVar.asName(),
						makeJavaType(typeFact().getBooleanType()), List.<JCTree.JCTypeParameter>nil(),
			...
			if (forIterator instanceof Tree.ValueIterator) {
				...
				error = makeErroneous(fcl,
						"compiler bug: iterators of type " + forIterator.getNodeType() + " not yet supported");
				return null;
			}
			...
			ListBuffer<JCStatement> contextBody = tx.makeContext(ExpressionTransformer.this, this, forIterator, iterVar,
					itemVar, tmpItem, elseBody);

			List<JCTree.JCStatement> methodBody;
			if (idx > 0) {
				// Subsequent iterators may depend on the item from the previous loop so we make
				// sure we have one
				methodBody = List.<JCStatement>of(
						make().WhileLoop(make().Apply(null, iterVar.makeIdentWithThis(), List.<JCExpression>nil()),
								make().Block(0, contextBody.toList())));
				// It can happen that we never get into the body because the outer iterator is
				// exhausted, if so, mark
				// this one exhausted too
				if (lastIteratorCtxtName != null) {
					// FIXME: this may actually not be useful to check for exhaustion because in
					// theory we can only get out
					// of the loop because the previous supplier is exhausted.
					methodBody = methodBody.append(make().If(
							lastIteratorCtxtName.suffixedBy(Suffix.$exhausted$).makeIdent(), make().Exec(make()
									.Assign(itemVar.suffixedBy(Suffix.$exhausted$).makeIdent(), makeBoolean(true))),
							null));
				} else {
					methodBody = methodBody.append(make().Exec(
							make().Assign(itemVar.suffixedBy(Suffix.$exhausted$).makeIdent(), makeBoolean(true))));
				}
			...
			fields.add(make().MethodDef(make().Modifiers(Flags.PRIVATE | Flags.FINAL), itemVar.asName(),
					makeJavaType(typeFact().getBooleanType()), List.<JCTree.JCTypeParameter>nil(),
					List.<JCTree.JCVariableDecl>nil(), List.<JCExpression>nil(), make().Block(0, methodBody), null));
			return itemVar;
		}
	}
	...
	private JCExpression unAutoPromote(JCExpression ret, Type returnType, boolean small) {
		...
		return applyJavaTypeConversions(ret, exprType, returnType, BoxingStrategy.UNBOXED, false, small, 0);
	}
	...
	boolean isFunctionalResult(Type type) {
		return !isWithinInvocation() && !type.isNothing() && isCeylonCallableSubtype(type);
	}

	boolean isJavaFunctionalInterfaceResult(Type type) {
		return !isWithinInvocation() && !type.isNothing() && checkForFunctionalInterface(type) != null;
	}
	...
	/*
	 * private JCExpression checkForArrayOnJavaArray(Tree.QualifiedMemberExpression
	 * expr) { if ("array".equals(expr.getIdentifier().getText())) { if
	 * (expr.getPrimary() instanceof Tree.BaseMemberExpression) { if
	 * (Decl.isJavaArray(expr.getPrimary().getTypeModel().getDeclaration())) {
	 * return transform((Tree.BaseMemberExpression)expr.getPrimary()); } } } return
	 * null; }
	 */

	boolean isThrowableSuppressed(Tree.QualifiedMemberOrTypeExpression expr) {
		return typeFact().getThrowableDeclaration().getDirectMember("suppressed", null, false)
				.equals(expr.getDeclaration().getRefinedDeclaration());
	}

	boolean isThrowableMessage(Tree.QualifiedMemberOrTypeExpression expr) {
		return typeFact().getThrowableDeclaration().getDirectMember("message", null, false)
				.equals(expr.getDeclaration().getRefinedDeclaration());
	}
	...
	private JCExpression checkForByteLiterals(Tree.QualifiedMemberExpression expr, boolean negative) {
		...
		if (!expr.getUnboxed() || !left.getUnboxed())
			return null;
		// This can't be Tree.NegativeOp as the normal precedence of -1.byte is
		// -(1.byte), not (-1).byte
	...
	private JCExpression checkForByteLiterals(Tree.InvocationExpression ce) {
		...
		if (ce.getPrimary() instanceof Tree.BaseTypeExpression && ce.getPositionalArgumentList() != null) {
			java.util.List<Tree.PositionalArgument> positionalArguments = ce.getPositionalArgumentList()
	...
	private JCExpression checkForBitwiseOperators(Tree.InvocationExpression ce) {
		...
		if (ce.getPositionalArgumentList() == null || ce.getPositionalArgumentList().getPositionalArguments() == null
	...
	private JCExpression checkForBitwiseOperators(Tree.Term node, Tree.QualifiedMemberExpression qme, Tree.Term right) {
		...
		if (operator != null) {
			...
			if (operator.getArity() == 2) {
				if (right == null)
					return null;
				OptimisationStrategy optimisationStrategy = operator.getBinOpOptimisationStrategy(node, left, right,
						this);
				...
				JCExpression leftExpr = transformExpression(left, optimisationStrategy.getBoxingStrategy(), binaryType,
						EXPR_WIDEN_PRIM);
				JCExpression rightExpr = transformExpression(right, optimisationStrategy.getBoxingStrategy(),
						binaryType, EXPR_WIDEN_PRIM);
	...
	/**
	 * Transform the annotations on the given annotated declaration for inclusion on
	 * the given target element type
	 */
	...
	/**
	 * Transform the annotations on the given package declaration for inclusion on
	 * the given target element type
	 */
	public List<JCAnnotation> transformAnnotations(OutputElement target, Tree.PackageDescriptor annotated) {
		return transform(annotated.getUnit().getPackage(), target, annotated.getAnnotationList(),
				AnnotationUtil.outputs((Tree.PackageDescriptor) annotated));
	}
	 ...
	 */
	public List<JCAnnotation> transformAnnotations(OutputElement target, Tree.ImportModule annotated) {
		return transform(annotated, target, annotated.getAnnotationList(),
				AnnotationUtil.outputs((Tree.ImportModule) annotated));
	}

	/**
	 * Transform the annotations on the given module declaration for inclusion on
	 * the given target element type
	 */
	public List<JCAnnotation> transformAnnotations(OutputElement target, Tree.ModuleDescriptor annotated) {
		return transform(annotated.getUnit().getPackage().getModule(), target, annotated.getAnnotationList(),
				AnnotationUtil.outputs((Tree.ModuleDescriptor) annotated));
	}

	private List<JCAnnotation> transform(Object useSite, OutputElement target, Tree.AnnotationList annotationList,
			EnumSet<OutputElement> outputs) {
		...
		if (annotationList != null) {
			...
			if (annotationList.getAnonymousAnnotation() != null && AnnotationUtil
			...
			if (annotationList.getAnnotations() != null) {
				for (Tree.Annotation annotation : annotationList.getAnnotations()) {
					Function annoCtorDecl = ((Function) ((Tree.BaseMemberExpression) annotation.getPrimary())
							.getDeclaration());
					if (annoCtorDecl != null) {
						String aname = annoCtorDecl.getQualifiedNameString();
						if ("java.lang::transient".equals(aname) || "java.lang::volatile".equals(aname)
								|| "java.lang::synchronized".equals(aname) || "java.lang::native".equals(aname)
								|| "java.lang::strictfp".equals(aname) || "java.lang::overloaded".equals(aname)
					...
					EnumSet<OutputElement> possibleTargets = AnnotationUtil.interopAnnotationTargeting(
							useSite instanceof Declaration ? isEe((Declaration) useSite) : false, outputs, annotation,
							false, false, useSite instanceof Declaration ? (Declaration) useSite : null);
					if ((isNaturalTarget && possibleTargets == null)
							|| (possibleTargets != null && possibleTargets.equals(EnumSet.of(target)))) {
		...
		for (Class annotationClass : annotationSet.keySet()) {
			...
			if (isSequencedAnnotation(annotationClass)) {
				...
				if (annotations.size() > 1) {
					makeErroneous(annotationList, "compiler bug: multiple occurances of non-sequenced annotation class "
							+ annotationClass.getQualifiedNameString());
				}
		...
		if (annotationList != null) {
			for (Tree.Annotation annotation : annotationList.getAnnotations()) {
				if (AnnotationUtil.isNaturalTarget((Function) typeFact().getLanguageModuleDeclaration("deprecated"),
						useSite, target) && isDeprecatedAnnotation(annotation.getPrimary())
						&& !(useSite instanceof Function) && !(useSite instanceof Constructor)) {
	...
	private boolean isDeprecatedAnnotation(Tree.Primary primary) {
		return primary instanceof Tree.BaseMemberExpression && typeFact().getLanguageModuleDeclaration("deprecated")
				.equals(((Tree.BaseMemberExpression) primary).getDeclaration());
	}

	private void putAnnotation(Map<Class, ListBuffer<JCAnnotation>> annotationSet, JCAnnotation annotation,
	...
	public void transformAnonymousAnnotation(Tree.AnonymousAnnotation annotation,
			Map<Class, ListBuffer<JCAnnotation>> annos) {
		HasErrorException firstExpressionErrorAndMarkBrokenness = errors()
				.getFirstExpressionErrorAndMarkBrokenness(annotation);
	...
	public static String getSerializedMetaLiteral(Tree.MetaLiteral ml) {
		if (ml instanceof Tree.ModuleLiteral) {
			...
			if (mod.getRestriction() || ip.getModel() == null) {
				return ip.getIdentifiers().isEmpty() ? ml.getUnit().getPackage().getModule().getNameAsString()
		...
		if (ml instanceof Tree.PackageLiteral) {
			...
			if (ip.getModel() == null) {
				return ip.getIdentifiers().isEmpty() ? ml.getUnit().getPackage().getNameAsString()
						: TreeUtil.formatPath(ip.getIdentifiers());
			}
		}
	...
	/*
	 * ref ::= version? module ; // note: version is optional to support looking up
	 * the // runtime version of a package, once we support this version ::= ':'
	 * SENTINEL ANYCHAR* SENTINEL ; module ::= dottedIdent package? ; dottedIdent
	 * ::= ident ('.' ident)* ; package ::= ':' ( relativePackage | absolutePackage
	 * ) ? ( ':' declaration ) ? ; // note: if no absolute or relative package
	 * given, it's the // root package of the module relativePackage ::= dottedIdent
	 * ; absolutePackage ::= '.' dottedIdent ; // note: to suport package names
	 * which don't start // with the module name declaration ::= type | function |
	 * value ; type ::= class | interface ; class ::= 'C' ident ( '.' member )?
	 * interface ::= 'I' ident ( '.' member )? member ::= declaration ; function ::=
	 * 'F' ident ; value ::= 'V' ident ; constructor ::= 'c' ident ;
	 */
	/**
	 * Appends into the given builder a String representation of the given module,
	 * suitable for parsing my the DeclarationParser.
	 */
	...
	/**
	 * Appends into the given builder a String representation of the given package,
	 * suitable for parsing my the DeclarationParser.
	 */
	...
	public JCTree transform(Tree.ObjectExpression expr) {
		...
		at(expr);
		JCExpression newCall = make().NewClass(null, null,
				makeUnquotedIdent(Naming.escapeClassName(expr.getAnonymousClass().getName()) + "_"),
	...
	private JCExpression transformIf(Tree.IfExpression op, Type expectedType) {
		...
		List<JCStatement> statements = statementGen().transformIf(conditions, thenPart, elseVar, elsePart, tmpVar, op,
				expectedType);
		at(op);
		// use the op model for the variable, not expected type, because expected type
		// may be optional, where op
		// says not optional (even in case of java interop which may return null), so we
		// allow null values in j.l.String (unboxed)
		// because the caller will insert the null check if the expected type is
		// optional
		Type typeModel = op.getTypeModel();
		if (willEraseToObject(typeModel)) {
			typeModel = typeFact().denotableType(typeModel);
		}
		JCExpression vartype = makeJavaType(typeModel,
				CodegenUtil.getBoxingStrategy(op) == BoxingStrategy.UNBOXED ? 0 : JT_NO_PRIMITIVES);
		return make().LetExpr(make().VarDef(make().Modifiers(0), names().fromString(tmpVar), vartype, null), statements,
	...
	private Type getExpectedTypeForJavaNullChecks(Term op, Type expectedType) {
		...
		if (expectedType == null || !isOptional(expectedType) || !containsUncheckedNulls(op))
