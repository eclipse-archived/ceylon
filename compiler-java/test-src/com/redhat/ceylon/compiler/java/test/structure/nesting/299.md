...
1. Change callsites for defaulted parameters to use the overloaded methods.
1. ??Refactoring to inline defaulted parameters where possible?? 
1. Stop generating `$impl` for classes, functions etc.
1. Stop making interface `$impl` static and move where we declare it
