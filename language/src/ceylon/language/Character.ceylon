...
shared final native class Character(Character character)
    ...
    "Determine if this character is a numeric digit.
     That is, if its Unicode general category is
     *DECIMAL_DIGIT_NUMBER*."
    shared native Boolean digit;

    "Determine if this character is a letter. That is,
     if its Unicode general category is *UPPERCASE_LETTER*,
     *LOWERCASE_LETTER*, *TITLECASE_LETTER*, *MODIFIER_LETTER*,
     or *OTHER_LETTER*."
     ...
     * U+001F *UNIT SEPARATOR*, and
     * any Unicode character in the general category *Zs* 
       (space separator), *Zl* (line separator) or *Zp* 
       (paragraph separator) that is not also a non-breaking 
       space.
     "
