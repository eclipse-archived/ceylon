void testIterables() {
    ...
    check(string(elements(for (c in "HoLa") c.uppercase then c else null).coalesced...)=="HL", "Iterable.coalesced");
    ...
    check(array().chain({1,2})=={1,2}, "ArrayOfNone.chain");
    check(array(1,2).chain({3,4}).sequence=={1,2,3,4}, "ArrayOfSome.chain");
