void comprehensions() {
    ...
    $error String joined1 = " ".join(for (w in wordsAnNulls) if (exists w) w.uppercased);
...
class OkSubWithIter(variable String s) {
    class Inner() extends SuperWithIter({for (c in s) c}) {}
}
